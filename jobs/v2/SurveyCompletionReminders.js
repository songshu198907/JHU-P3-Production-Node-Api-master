module.exports = {
  ...
  fn: function(done) {
    ...
    var lCustomQuery="select us.*,u.first_name,u.last_name,u.email,u.cell_phone from user_surveys as us,users as u where us.is_complete = 0 and us.first_reminder = 0 and us.second_reminder = 0 "+
                    "and DATE(us.created_at) <= (curdate()- INTERVAL 5 DAY) and us.user_id=u.id "+
                    ...
                    "and DATE(us.created_at) <= (curdate()- INTERVAL 10 DAY) and us.user_id=u.id "+
                    "and us.user_id in (select id from users where is_deactive = 0) "+
                    "union all "+
                    "select us.*,u.first_name,u.last_name,u.email,u.cell_phone from user_surveys as us,users as u where us.is_complete = 0 and us.first_reminder = 1 and us.second_reminder = 1 "+
                    "and DATE(us.created_at) = curdate()- INTERVAL 15 DAY and us.user_id=u.id "+
        ...
        .exec(function(err,response){
          if(err){
            ...
            _.each(pSurveys, function(lSurvey, index) {
              if(lSurvey.first_reminder && lSurvey.second_reminder && lSurvey.cell_phone){
              ...
              }else if(!lSurvey.first_reminder || !lSurvey.second_reminder){
    ...
    function notify(message,pLastMessage) {
      ...
      if(Model !== undefined && Model.request) {
        Promise.promisify(Model.request)('create', {}, {}, { body: message.body, env: process.env })
            .then(function(resultMessage) {
                sails.log.debug(message.model + ' responding with: ' + util.inspect(resultMessage));
                if(message.survey.first_reminder){//if first reminder has been sent
                  updateSurveyReminder(message.survey,{second_reminder:1},pLastMessage);
                }else{
                  updateSurveyReminder(message.survey,{first_reminder:1},pLastMessage);
                }
