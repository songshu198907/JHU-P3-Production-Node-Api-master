module.exports = {
  ...
  if(req.query.query!==undefined && req.query.query.email!==undefined){
      ...
      var lSearchCriteria={email:lUserEmail,is_deactive:false};
    lUserModel.findOne(lSearchCriteria,function(err,result){
      if(err){
        req.res.json(401,{"message":"unable to find user."});
        req.res.end();
        return;
      }
      if(result!==undefined){
        ...
        function updateUserPassword(){
          ...
          lUserModel.update(lSearchCriteria,{passwordDigest:result.passwordDigest,resetPassword:true})
          .then(function(result) {
            sails.log.debug('User password updated: ' + util.inspect(result));
            if(result!==undefined && result.length>0){
              var settingsModel=self.models("setting");
              settingsModel.find({},{fields: {forgotPasswordEmailText:1,forgotPasswordSubject:1,fromEmail:1}}).paginate({page: 1}, {limit: 1})
              ...
              req.res.json(401,{message:"password reset failed"});
              req.res.end();
              return;
            }
          })
    ...
    function sendPasswordToUser(pToEmail,pPassword,pBody,pSubject,pFromEmail){
      ...
      var finalTemplate=template({password: pPassword});
      ...
      Promise.promisify(SendGrid.request)('create', {}, {}, {body:msgBody})
        .then(function(resultSendGrid) {
          ...
          req.res.json(200,{message:"password reset successful"});
          req.res.end();
          return;
        })
        .catch(function(err) {
    ...
    function hashPassword(values,next){
      if (values.password) {
        ...
        bcrypt.hash(values.password, 10, function(err, res) {
          if (err) {
            req.res.json(401,{message:"unable to hash password"});
            req.res.end();
            return;
          }
    ...
    req.res.json(401,{message:"email parameter missing ...."});
    req.res.end();
    return;
  }
  },
...
var util=require("util");
     if(req.context.current_user.role==="patient" && req.context.current_user.patientType===2){
        ...
        if(req.context.current_user.consentAcceptedOn!==null){
          req.res.json(401,{"error":"consent already accepted for this user."});
          return false;
        }
        var userModel= this.models('user');
        userModel.update({id:req.context.current_user.id},{consent_accepted_on:new Date()})
 ...
 var self=this;
  ...
  function deActiveUser(pEmail){
    if((req.query && (_.isNull(req.query.reason_for_deactivation) || _.isUndefined(req.query.reason_for_deactivation))) && req.context.current_user.role!="administrator"){
      req.res.json(401,{"message":"Missing reason for deactivation parameter."});
      req.res.end();
      return false;
    }
    ...
    UserModel.update({"email":pEmail},{"is_deactive":true,"vaccination_reminders":false,"reason_for_deactivation":reasonForDeactivation,"deactivatedOn":(new Date())},function(error,response){
      if(error){
        req.res.json(401,{"error":"failed to deactivate user"});
        return false;
