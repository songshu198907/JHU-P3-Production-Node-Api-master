...
V2JobAuditLogging = module.exports = {
  migrate: 'safe',
  ...
  autoUpdatedAt: false,
    ...
    var scope = V2JobAuditLogging.find().where();
    if(limit) {
  ...
  ,
    ...
    var scope = V2JobAuditLogging.find().where(transformAttributesForExactMatch(attributes));
    if(limit) {
      scope = scope.limit(limit);
    }
    if(offset) {
  ...
  ,
    ...
    return V2JobAuditLogging.count(transformAttributesForExactMatch(attributes));
...
// Augment/override definition of V2JobAuditLogging using customizations provided via custom code
if(extensions) {
  extend(V2JobAuditLogging, 'V2JobAuditLogging', extensions, 'customCode');
}

if (callbacks) {
  extend(V2JobAuditLogging, 'V2JobAuditLogging', callbacks, 'callbacks');
}
