...
V1Video = module.exports = {
  ...
  ,
    ...
    var scope = V1Video.find().where(transformAttributesForExactMatch(attributes));
    if(limit) {
      scope = scope.limit(limit);
    }
    if(offset) {
  ...
  ,
    ...
    return V1Video.count(transformAttributesForExactMatch(attributes));
  ...
  ,
    ...
    var criteria = new Criteria(V1Video);
...
// Augment/override definition of V1Video using customizations provided via custom code
if(extensions) {
  extend(V1Video, 'V1Video', extensions, 'customCode');
}

if (callbacks) {
  extend(V1Video, 'V1Video', callbacks, 'callbacks');
}
