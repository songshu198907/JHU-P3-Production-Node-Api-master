...
V1Race = module.exports = {
  ...
  ,
    ...
    var scope = V1Race.find().where(transformAttributesForExactMatch(attributes));
    if(limit) {
      scope = scope.limit(limit);
    }
    if(offset) {
  ...
  ,
    ...
    return V1Race.count(transformAttributesForExactMatch(attributes));
  ...
  ,
    ...
    var criteria = new Criteria(V1Race);
...
// Augment/override definition of V1Race using customizations provided via custom code
if(extensions) {
  extend(V1Race, 'V1Race', extensions, 'customCode');
}

if (callbacks) {
  extend(V1Race, 'V1Race', callbacks, 'callbacks');
}
