...
V1Setting = module.exports = {
  ...
  autoUpdatedAt: false,
    ...
    var scope = V1Setting.find().where();
    if(limit) {
  ...
  ,
    ...
    var scope = V1Setting.find().where(transformAttributesForExactMatch(attributes));
    if(limit) {
      scope = scope.limit(limit);
    }
    if(offset) {
  ...
  ,
    ...
    return V1Setting.count(transformAttributesForExactMatch(attributes));
...
// Augment/override definition of V1Setting using customizations provided via custom code
if(extensions) {
  extend(V1Setting, 'V1Setting', extensions, 'customCode');
}

if (callbacks) {
  extend(V1Setting, 'V1Setting', callbacks, 'callbacks');
}
