var extensions          = require('./custom/v1/surveyQuestionCustom'),
    callbacks           = require('./custom/v1/surveyQuestionCallbacks'),
...
V1SurveyQuestion = module.exports = {
  migrate: 'safe',
  ...
  toJSON: function () {
    ...
    surveyAnswers: {
      collection: 'V1SurveyAnswer',
      via: 'surveyQuestion'
    },
  
    userSurveyAnswers: {
      collection: 'V1UserSurveyAnswer',
      via: 'surveyQuestion'
    },
  ...
  autoUpdatedAt: false,
    ...
    var scope = V1SurveyQuestion.find().where();
    if(limit) {
  ...
  ,
    ...
    var scope = V1SurveyQuestion.find().where(transformAttributesForExactMatch(attributes));
    if(limit) {
      scope = scope.limit(limit);
    }
    if(offset) {
  ...
  ,
    ...
    return V1SurveyQuestion.count(transformAttributesForExactMatch(attributes));
  ...
  ,
    ...
    var criteria = new Criteria(V1SurveyQuestion);
...
// Augment/override definition of V1SurveyQuestion using customizations provided via custom code
if(extensions) {
  extend(V1SurveyQuestion, 'V1SurveyQuestion', extensions, 'customCode');
}

if (callbacks) {
  extend(V1SurveyQuestion, 'V1SurveyQuestion', callbacks, 'callbacks');
}
