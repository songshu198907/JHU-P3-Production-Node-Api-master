var extensions          = require('./custom/v1/userSurveyCustom'),
    callbacks           = require('./custom/v1/userSurveyCallbacks'),
...
var transformAttributesForExactMatch = function(attributes) {
  ...
  if(undefined != attributes['maternal_video_number']) {
    newAttributes['maternalVideoNumber'] = Number(attributes['maternal_video_number']);
  }
  ...
  if(undefined != attributes['pediatric_video_number']) {
    newAttributes['pediatricVideoNumber'] = Number(attributes['pediatric_video_number']);
  }
...
'matchedVideos'
  ...
  autoUpdatedAt: false,
    ...
    var scope = V1UserSurvey.find().where();
    if(limit) {
  ...
  ,
    ...
    var scope = V1UserSurvey.find().where(transformAttributesForExactMatch(attributes));
    if(limit) {
      scope = scope.limit(limit);
    }
    if(offset) {
  ...
  ,
    ...
    return V1UserSurvey.count(transformAttributesForExactMatch(attributes));
  ...
  ,
    ...
    var criteria = new Criteria(V1UserSurvey);
...
// Augment/override definition of V1UserSurvey using customizations provided via custom code
if(extensions) {
  extend(V1UserSurvey, 'V1UserSurvey', extensions, 'customCode');
}

if (callbacks) {
  extend(V1UserSurvey, 'V1UserSurvey', callbacks, 'callbacks');
}
