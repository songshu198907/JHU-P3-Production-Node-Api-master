var extensions          = require('./custom/v1/questionCodeCustom'),
    callbacks           = require('./custom/v1/questionCodeCallbacks'),
...
V1QuestionCode = module.exports = {
  ...
  autoUpdatedAt: false,
    ...
    var scope = V1QuestionCode.find().where();
    if(limit) {
  ...
  ,
    ...
    var scope = V1QuestionCode.find().where(transformAttributesForExactMatch(attributes));
    if(limit) {
      scope = scope.limit(limit);
    }
    if(offset) {
  ...
  ,
    ...
    return V1QuestionCode.count(transformAttributesForExactMatch(attributes));
  ...
  ,
    ...
    var criteria = new Criteria(V1QuestionCode);
...
// Augment/override definition of V1QuestionCode using customizations provided via custom code
if(extensions) {
  extend(V1QuestionCode, 'V1QuestionCode', extensions, 'customCode');
}

if (callbacks) {
  extend(V1QuestionCode, 'V1QuestionCode', callbacks, 'callbacks');
}
