var extensions          = require('./custom/v1/topicCustom'),
    callbacks           = require('./custom/v1/topicCallbacks'),
...
V1Topic = module.exports = {
  ...
  toJSON: function () {
    ...
    healthcareProviderContents: {
      collection: 'V1HealthcareProviderContent',
      via: 'topic'
    },
  ...
  ,
    ...
    var scope = V1Topic.find().where(transformAttributesForExactMatch(attributes));
    if(limit) {
      scope = scope.limit(limit);
    }
    if(offset) {
  ...
  ,
    ...
    return V1Topic.count(transformAttributesForExactMatch(attributes));
  ...
  ,
    ...
    var criteria = new Criteria(V1Topic);
...
// Augment/override definition of V1Topic using customizations provided via custom code
if(extensions) {
  extend(V1Topic, 'V1Topic', extensions, 'customCode');
}

if (callbacks) {
  extend(V1Topic, 'V1Topic', callbacks, 'callbacks');
}
