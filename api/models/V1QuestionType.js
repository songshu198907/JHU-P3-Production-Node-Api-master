var extensions          = require('./custom/v1/questionTypeCustom'),
    callbacks           = require('./custom/v1/questionTypeCallbacks'),
...
V1QuestionType = module.exports = {
  ...
  autoUpdatedAt: false,
    ...
    var scope = V1QuestionType.find().where();
    if(limit) {
  ...
  ,
    ...
    var scope = V1QuestionType.find().where(transformAttributesForExactMatch(attributes));
    if(limit) {
      scope = scope.limit(limit);
    }
    if(offset) {
  ...
  ,
    ...
    return V1QuestionType.count(transformAttributesForExactMatch(attributes));
  ...
  ,
    ...
    var criteria = new Criteria(V1QuestionType);
...
// Augment/override definition of V1QuestionType using customizations provided via custom code
if(extensions) {
  extend(V1QuestionType, 'V1QuestionType', extensions, 'customCode');
}

if (callbacks) {
  extend(V1QuestionType, 'V1QuestionType', callbacks, 'callbacks');
}
