...
V1VaccinationReminder = module.exports = {
  migrate: 'safe',
  ...
  autoUpdatedAt: false,
    ...
    var scope = V1VaccinationReminder.find().where();
    if(limit) {
  ...
  ,
    ...
    var scope = V1VaccinationReminder.find().where(transformAttributesForExactMatch(attributes));
    if(limit) {
      scope = scope.limit(limit);
    }
    if(offset) {
  ...
  ,
    ...
    return V1VaccinationReminder.count(transformAttributesForExactMatch(attributes));
  ...
  ,
    ...
    var criteria = new Criteria(V1VaccinationReminder);
...
// Augment/override definition of V1VaccinationReminder using customizations provided via custom code
if(extensions) {
  extend(V1VaccinationReminder, 'V1VaccinationReminder', extensions, 'customCode');
}

if (callbacks) {
  extend(V1VaccinationReminder, 'V1VaccinationReminder', callbacks, 'callbacks');
}
