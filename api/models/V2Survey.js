var extensions          = require('./custom/v2/surveyCustom'),
    callbacks           = require('./custom/v2/surveyCallbacks'),
...
V2Survey = module.exports = {
  ...
  ,
    ...
    var scope = V2Survey.find().where(transformAttributesForExactMatch(attributes));
    if(limit) {
      scope = scope.limit(limit);
    }
    if(offset) {
  ...
  ,
    ...
    return V2Survey.count(transformAttributesForExactMatch(attributes));
  ...
  ,
    ...
    var criteria = new Criteria(V2Survey);
...
// Augment/override definition of V2Survey using customizations provided via custom code
if(extensions) {
  extend(V2Survey, 'V2Survey', extensions, 'customCode');
}

if (callbacks) {
  extend(V2Survey, 'V2Survey', callbacks, 'callbacks');
}
