...
V2ClinicianBlockRandomizer = module.exports = {
  migrate: 'safe',
  ...
  autoUpdatedAt: false,
    ...
    var scope = V2ClinicianBlockRandomizer.find().where();
    if(limit) {
  ...
  ,
    ...
    var scope = V2ClinicianBlockRandomizer.find().where(transformAttributesForExactMatch(attributes));
    if(limit) {
      scope = scope.limit(limit);
    }
    if(offset) {
  ...
  ,
    ...
    return V2ClinicianBlockRandomizer.count(transformAttributesForExactMatch(attributes));
  ...
  ,
    ...
    var criteria = new Criteria(V2ClinicianBlockRandomizer);
...
// Augment/override definition of V2ClinicianBlockRandomizer using customizations provided via custom code
if(extensions) {
  extend(V2ClinicianBlockRandomizer, 'V2ClinicianBlockRandomizer', extensions, 'customCode');
}

if (callbacks) {
  extend(V2ClinicianBlockRandomizer, 'V2ClinicianBlockRandomizer', callbacks, 'callbacks');
}
