var extensions          = require('./custom/v2/healthcareProviderContentCustom'),
    callbacks           = require('./custom/v2/healthcareProviderContentCallbacks'),
...
V2HealthcareProviderContent = module.exports = {
  migrate: 'safe',
  ...
  autoUpdatedAt: false,
    ...
    var scope = V2HealthcareProviderContent.find().where();
    if(limit) {
  ...
  ,
    ...
    var scope = V2HealthcareProviderContent.find().where(transformAttributesForExactMatch(attributes));
    if(limit) {
      scope = scope.limit(limit);
    }
    if(offset) {
  ...
  ,
    ...
    return V2HealthcareProviderContent.count(transformAttributesForExactMatch(attributes));
  ...
  ,
    ...
    var criteria = new Criteria(V2HealthcareProviderContent);
...
// Augment/override definition of V2HealthcareProviderContent using customizations provided via custom code
if(extensions) {
  extend(V2HealthcareProviderContent, 'V2HealthcareProviderContent', extensions, 'customCode');
}

if (callbacks) {
  extend(V2HealthcareProviderContent, 'V2HealthcareProviderContent', callbacks, 'callbacks');
}
