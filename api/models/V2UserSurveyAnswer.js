var extensions          = require('./custom/v2/userSurveyAnswerCustom'),
    callbacks           = require('./custom/v2/userSurveyAnswerCallbacks'),
...
V2UserSurveyAnswer = module.exports = {
  migrate: 'safe',
...
'questionGroup'
  ...
  autoUpdatedAt: false,
    ...
    var scope = V2UserSurveyAnswer.find().where();
    if(limit) {
  ...
  ,
    ...
    var scope = V2UserSurveyAnswer.find().where(transformAttributesForExactMatch(attributes));
    if(limit) {
      scope = scope.limit(limit);
    }
    if(offset) {
  ...
  ,
    ...
    return V2UserSurveyAnswer.count(transformAttributesForExactMatch(attributes));
  ...
  ,
    ...
    var criteria = new Criteria(V2UserSurveyAnswer);
...
// Augment/override definition of V2UserSurveyAnswer using customizations provided via custom code
if(extensions) {
  extend(V2UserSurveyAnswer, 'V2UserSurveyAnswer', extensions, 'customCode');
}

if (callbacks) {
  extend(V2UserSurveyAnswer, 'V2UserSurveyAnswer', callbacks, 'callbacks');
}
