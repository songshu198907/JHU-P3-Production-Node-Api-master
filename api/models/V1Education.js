var extensions          = require('./custom/v1/educationCustom'),
    callbacks           = require('./custom/v1/educationCallbacks'),
...
V1Education = module.exports = {
  ...
  autoUpdatedAt: false,
    ...
    var scope = V1Education.find().where();
    if(limit) {
  ...
  ,
    ...
    var scope = V1Education.find().where(transformAttributesForExactMatch(attributes));
    if(limit) {
      scope = scope.limit(limit);
    }
    if(offset) {
  ...
  ,
    ...
    return V1Education.count(transformAttributesForExactMatch(attributes));
  ...
  ,
    ...
    var criteria = new Criteria(V1Education);
...
// Augment/override definition of V1Education using customizations provided via custom code
if(extensions) {
  extend(V1Education, 'V1Education', extensions, 'customCode');
}

if (callbacks) {
  extend(V1Education, 'V1Education', callbacks, 'callbacks');
}
