...
V1SearchAuditLogging = module.exports = {
  migrate: 'safe',
  ...
  autoUpdatedAt: false,
    ...
    var scope = V1SearchAuditLogging.find().where();
    if(limit) {
  ...
  ,
    ...
    var scope = V1SearchAuditLogging.find().where(transformAttributesForExactMatch(attributes));
    if(limit) {
      scope = scope.limit(limit);
    }
    if(offset) {
  ...
  ,
    ...
    return V1SearchAuditLogging.count(transformAttributesForExactMatch(attributes));
...
// Augment/override definition of V1SearchAuditLogging using customizations provided via custom code
if(extensions) {
  extend(V1SearchAuditLogging, 'V1SearchAuditLogging', extensions, 'customCode');
}

if (callbacks) {
  extend(V1SearchAuditLogging, 'V1SearchAuditLogging', callbacks, 'callbacks');
}
