...
V1VideoAuditLogging = module.exports = {
  migrate: 'safe',
  ...
  autoUpdatedAt: false,
    ...
    var scope = V1VideoAuditLogging.find().where();
    if(limit) {
  ...
  ,
    ...
    var scope = V1VideoAuditLogging.find().where(transformAttributesForExactMatch(attributes));
    if(limit) {
      scope = scope.limit(limit);
    }
    if(offset) {
  ...
  ,
    ...
    return V1VideoAuditLogging.count(transformAttributesForExactMatch(attributes));
  ...
  ,
    ...
    var criteria = new Criteria(V1VideoAuditLogging);
...
// Augment/override definition of V1VideoAuditLogging using customizations provided via custom code
if(extensions) {
  extend(V1VideoAuditLogging, 'V1VideoAuditLogging', extensions, 'customCode');
}

if (callbacks) {
  extend(V1VideoAuditLogging, 'V1VideoAuditLogging', callbacks, 'callbacks');
}
