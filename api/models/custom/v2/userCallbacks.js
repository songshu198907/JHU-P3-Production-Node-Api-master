...
if (process.env.NODE_ENV && process.env.NODE_ENV === 'test') return done();
  ...
  if (process.env.NODE_ENV && process.env.NODE_ENV === 'test') return done();
    ...
    function createEnrollmentId(){
      ...
      models[0].enrollmentId=req.context.body.clinician_code+month+day+models[0].id;
      console.log(models[0].enrollmentId);
      var UserModel=self.models("user");
      UserModel.update({id:models[0].id},{enrollment_id:models[0].enrollmentId}).exec(function afterwards(err, updated){
...
var self=this;
 ...
 if (process.env.NODE_ENV && process.env.NODE_ENV === 'test') return done();
  ...
  beforeGetUsersByRaceScope: function(values, req, done) {
    
    done();
  },
  afterGetUsersByRaceScope: function(models, req, done) {
    done();
  },

  beforeGetUsersByEducationScope: function(values, req, done) {
    
    done();
  },
  afterGetUsersByEducationScope: function(models, req, done) {
    done();
  },  
