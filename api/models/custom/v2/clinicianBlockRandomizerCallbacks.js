module.exports = {
  beforeCreate: function(values, req, done) {
    // create the students
    var lCount=req.context.body.create_count;
    if(lCount && lCount>1){
      for(var i=1;i<lCount;i++){
        var blockRandomizerModel=this.models("clinicianBlockRandomizer");
        console.log(values);
        blockRandomizerModel.create(values).exec(function createCB(err, created){
          if(err){
            console.log("multiple block randomizer creation failed");
          }else if(created ){
            console.log("multile block randomizers created");
          }
        });  
      }
    }
