...
this.models('video').update({"id":req.params.id},{"is_active":0},function(err,result){
  ...
  afterAllScope: function(models, req, done) {
    ...
    models && _.each(models,function(video){
      if(video.videoUrl && video.videoUrl!==""){
        video.videoUrl=process.env.AWS_CLOUD_FRONT_URL+encodeURIComponent(decodeURIComponent(video.videoUrl)).split("%2F").pop();
      }
        ...
        if(result && result.length>0 && result[0].video_source_version){
          ...
          _.each(models,function(model){
            if(model.sourceVersion == result[0].video_source_version){
              if(model.videoUrl && model.videoUrl!==""){
                model.videoUrl=process.env.AWS_CLOUD_FRONT_URL+encodeURIComponent(decodeURIComponent(model.videoUrl)).split("%2F").pop();
              }
