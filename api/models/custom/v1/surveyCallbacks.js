      ...
      self.models('survey').query(surveyInsertSQL, function functionAnswerDeleted(err, addedSurvey){
          ...
          self.models('SurveyQuestion').query(questionInsertSQL, function functionAnswerDeleted(err, addedQuestions){
            ...
            if(addedQuestions && addedQuestions.affectedRows>0 && addedQuestions.insertId){
                ...
                var selectSurveyQuestionDifference ="select a.id, b.id, a.id - b.id as difference from survey_questions  a , survey_questions b "+
                          "where  a.survey_id ="+copySurveyId+
                          " and a.sort_order = 1"+
                          " and b.survey_id = "+req.query.query.survey_id+
                          " and b.sort_order = 1 ;";
                self.models('SurveyQuestion').query(selectSurveyQuestionDifference,function(err,result){
                  
                    if(result && result.length>0){
                        var questionDifference=result[0].difference;
                        
                        var answerInsertSQL ="insert into survey_answers (survey_question_id, allow_free_form, label, weight, sort_order, free_form_data_type, video_target_number)"+
                        "select survey_question_id +"+result[0].difference+", allow_free_form, label, weight, sort_order, free_form_data_type, video_target_number "+
                         "from survey_answers where survey_question_id in ("+
                         "select id from survey_questions where survey_id = "+req.query.query.survey_id+");";
                        ...
                        self.models('SurveyAnswer').query(answerInsertSQL, function functionAnswerDeleted(err, addedAnswers){
                          ...
                          {
                              var selectSkipLogic="select survey_answers.id, survey_question_id, sort_order, skip_question_codes "+
                              "from survey_answers , survey_question_skip_logic "+
                              "where survey_question_id in ( "+
                              "select id from survey_questions where survey_id = "+req.query.query.survey_id+") "+
                              "and survey_answers.id=survey_question_skip_logic.survey_answer_id; ";
                             
                              self.models('SurveyAnswer').query(selectSkipLogic, function functionAnswerDeleted(err, skipResults){
                                if(err){
                                  done(err);
                                }
                                
                                if(skipResults && skipResults.length>0){
                                  _.each(skipResults,function(result,index){
                                    var questionId=result.survey_question_id+questionDifference;
                                    
                                    var insertSkipLogic="insert into survey_question_skip_logic (survey_answer_id, skip_question_codes) values "+
                                    "((select id from survey_answers where survey_question_id in (select survey_question_id from survey_answers where survey_question_id="+questionId+") and sort_order in (select sort_order from survey_answers where id="+result.id+")),'"+result.skip_question_codes +"')";
                                    self.models('SurveyAnswer').query(insertSkipLogic, function functionAnswerDeleted(err, skipResults){
                                      if(err){
                                        done(err);
                                      }
                                    });
                                    if(index==skipResults.length-1){
                                      req.res.json(200,{});
                                      return false;
                                    }
                                  });
