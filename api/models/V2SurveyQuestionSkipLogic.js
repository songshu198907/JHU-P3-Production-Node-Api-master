var extensions          = require('./custom/v2/surveyQuestionSkipLogicCustom'),
    callbacks           = require('./custom/v2/surveyQuestionSkipLogicCallbacks'),
...
V2SurveyQuestionSkipLogic = module.exports = {
  migrate: 'safe',
  ...
  autoUpdatedAt: false,
    ...
    var scope = V2SurveyQuestionSkipLogic.find().where();
    if(limit) {
  ...
  ,
    ...
    var scope = V2SurveyQuestionSkipLogic.find().where(transformAttributesForExactMatch(attributes));
    if(limit) {
      scope = scope.limit(limit);
    }
    if(offset) {
  ...
  ,
    ...
    return V2SurveyQuestionSkipLogic.count(transformAttributesForExactMatch(attributes));
  ...
  ,
    ...
    var criteria = new Criteria(V2SurveyQuestionSkipLogic);
...
// Augment/override definition of V2SurveyQuestionSkipLogic using customizations provided via custom code
if(extensions) {
  extend(V2SurveyQuestionSkipLogic, 'V2SurveyQuestionSkipLogic', extensions, 'customCode');
}

if (callbacks) {
  extend(V2SurveyQuestionSkipLogic, 'V2SurveyQuestionSkipLogic', callbacks, 'callbacks');
}
