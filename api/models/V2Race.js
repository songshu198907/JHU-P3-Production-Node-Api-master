...
V2Race = module.exports = {
  ...
  ,
    ...
    var scope = V2Race.find().where(transformAttributesForExactMatch(attributes));
    if(limit) {
      scope = scope.limit(limit);
    }
    if(offset) {
  ...
  ,
    ...
    return V2Race.count(transformAttributesForExactMatch(attributes));
  ...
  ,
    ...
    var criteria = new Criteria(V2Race);
...
// Augment/override definition of V2Race using customizations provided via custom code
if(extensions) {
  extend(V2Race, 'V2Race', extensions, 'customCode');
}

if (callbacks) {
  extend(V2Race, 'V2Race', callbacks, 'callbacks');
}
