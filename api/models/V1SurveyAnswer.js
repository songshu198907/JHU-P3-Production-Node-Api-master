var extensions          = require('./custom/v1/surveyAnswerCustom'),
    callbacks           = require('./custom/v1/surveyAnswerCallbacks'),
...
V1SurveyAnswer = module.exports = {
  ...
  autoUpdatedAt: false,
    ...
    var scope = V1SurveyAnswer.find().where();
    if(limit) {
  ...
  ,
    ...
    var scope = V1SurveyAnswer.find().where(transformAttributesForExactMatch(attributes));
    if(limit) {
      scope = scope.limit(limit);
    }
    if(offset) {
  ...
  ,
    ...
    return V1SurveyAnswer.count(transformAttributesForExactMatch(attributes));
  ...
  ,
    ...
    var criteria = new Criteria(V1SurveyAnswer);
...
// Augment/override definition of V1SurveyAnswer using customizations provided via custom code
if(extensions) {
  extend(V1SurveyAnswer, 'V1SurveyAnswer', extensions, 'customCode');
}

if (callbacks) {
  extend(V1SurveyAnswer, 'V1SurveyAnswer', callbacks, 'callbacks');
}
