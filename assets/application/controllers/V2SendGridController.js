...
angular.module('adminConsole.controllers.V2SendGridControllers', ['adminConsole.services'])

  .controller('V2SendGridListController', ['$scope', '$location', '$route', '$log', 'Utilities', 'V2SendGrid', 'httpErrorHandler', 'pagination', function($scope, $location, $route, $log, Utilities, V2SendGrid, httpErrorHandler, pagination) {
    $scope.error = false;
  ...
  .controller('V2SendGridDetailController', ['$scope', '$location', '$log', '$routeParams', 'Utilities', 'V2SendGrid', 'httpErrorHandler', function($scope, $location, $log, $routeParams, Utilities, V2SendGrid, httpErrorHandler) {
    $scope.linkClass = Utilities.linkClass;
    $scope.objectId = $routeParams.objectId;

    V2SendGrid.get({id: $routeParams.objectId}).$promise
      .then(function(val) {
        $scope.instance = val;
      })
      .catch(httpErrorHandler($scope));
  ...
  .controller('V2SendGridCreateController', ['$scope', '$location', '$log', '$routeParams', 'Utilities', 'V2SendGrid', 'httpErrorHandler', '$http', function($scope, $location, $log, $routeParams, Utilities, V2SendGrid, httpErrorHandler, $http) {
    $scope.linkClass = Utilities.linkClass;
    $scope.instance = {};
    $scope.loading = false;
    ...
    $scope.submit = function(instance) {
      ...
      $http.post("/api/v2/send_grids", fd, {
  ...
  .controller('V2SendGridEditController', ['$scope', '$location', '$log', '$routeParams', 'Utilities', 'V2SendGrid', 'httpErrorHandler', '$http', function($scope, $location, $log, $routeParams, Utilities, V2SendGrid, httpErrorHandler, $http) {
    $scope.linkClass = Utilities.linkClass;
    $scope.objectId = $routeParams.objectId;
    $scope.loading = false;
    ...
    $scope.submit = function(instance) {
      ...
      $http.put("/api/v2/send_grids/"+instance.id, fd, {
          transformRequest: function(data) { return data; },
          headers: {'Content-Type': undefined}
        })
        .success(function(d) {
