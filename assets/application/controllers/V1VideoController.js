...
angular.module('adminConsole.controllers.V1VideoControllers', ['adminConsole.services'])

  .controller('V1VideoListController', ['$scope', '$location', '$route', '$log', 'Utilities', 'V1Video', 'httpErrorHandler', 'pagination', function($scope, $location, $route, $log, Utilities, V1Video, httpErrorHandler, pagination) {
    $scope.error = false;
  ...
  .controller('V1VideoDetailController', ['$scope', '$location', '$log', '$routeParams', 'Utilities', 'V1Video', 'httpErrorHandler', function($scope, $location, $log, $routeParams, Utilities, V1Video, httpErrorHandler) {
    $scope.linkClass = Utilities.linkClass;
    $scope.objectId = $routeParams.objectId;

    V1Video.get({id: $routeParams.objectId}).$promise
      .then(function(val) {
        $scope.instance = val;
      })
      .catch(httpErrorHandler($scope));
  ...
  .controller('V1VideoCreateController', ['$scope', '$location', '$log', '$routeParams', 'Utilities', 'V1Video', 'httpErrorHandler', '$http', function($scope, $location, $log, $routeParams, Utilities, V1Video, httpErrorHandler, $http) {
    $scope.linkClass = Utilities.linkClass;
    $scope.instance = {};
    $scope.loading = false;
  ...
  .controller('V1VideoEditController', ['$scope', '$location', '$log', '$routeParams', 'Utilities', 'V1Video', 'httpErrorHandler', '$http', function($scope, $location, $log, $routeParams, Utilities, V1Video, httpErrorHandler, $http) {
    $scope.linkClass = Utilities.linkClass;
    $scope.objectId = $routeParams.objectId;
    $scope.loading = false;
    ...
    $scope.submit = function(instance) {
      ...
      $http.put("/api/v1/videos/"+instance.id, fd, {
          transformRequest: function(data) { return data; },
          headers: {'Content-Type': undefined}
        })
        .success(function(d) {
