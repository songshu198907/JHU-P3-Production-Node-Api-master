...
angular.module('adminConsole.controllers.V2SurveyControllers', ['adminConsole.services'])

  .controller('V2SurveyListController', ['$scope', '$location', '$route', '$log', 'Utilities', 'V2Survey', 'httpErrorHandler', 'pagination', function($scope, $location, $route, $log, Utilities, V2Survey, httpErrorHandler, pagination) {
    $scope.error = false;
  ...
  .controller('V2SurveyDetailController', ['$scope', '$location', '$log', '$routeParams', 'Utilities', 'V2Survey', 'httpErrorHandler', function($scope, $location, $log, $routeParams, Utilities, V2Survey, httpErrorHandler) {
    $scope.linkClass = Utilities.linkClass;
    $scope.objectId = $routeParams.objectId;

    V2Survey.get({id: $routeParams.objectId}).$promise
      .then(function(val) {
        $scope.instance = val;
      })
      .catch(httpErrorHandler($scope));
  ...
  .controller('V2SurveyCreateController', ['$scope', '$location', '$log', '$routeParams', 'Utilities', 'V2Survey', 'httpErrorHandler', '$http', function($scope, $location, $log, $routeParams, Utilities, V2Survey, httpErrorHandler, $http) {
    $scope.linkClass = Utilities.linkClass;
    $scope.instance = {};
    $scope.loading = false;
  ...
  .controller('V2SurveyEditController', ['$scope', '$location', '$log', '$routeParams', 'Utilities', 'V2Survey', 'httpErrorHandler', '$http', function($scope, $location, $log, $routeParams, Utilities, V2Survey, httpErrorHandler, $http) {
    $scope.linkClass = Utilities.linkClass;
    $scope.objectId = $routeParams.objectId;
    $scope.loading = false;
    ...
    $scope.submit = function(instance) {
      ...
      $http.put("/api/v2/surveys/"+instance.id, fd, {
          transformRequest: function(data) { return data; },
          headers: {'Content-Type': undefined}
        })
        .success(function(d) {
