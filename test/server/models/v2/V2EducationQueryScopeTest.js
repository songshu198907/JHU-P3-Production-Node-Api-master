...
describe('V2Education query scopes', function() {

  beforeEach( function(done){
	objects = [];
var testObjects = [];
  testObjects.push({"name":"Leslie Gerlach"});
  testObjects.push({"name":"Refugia Lesch"});
  testObjects.push({"name":"Mechelle Erdman"});
...
V2Education.createEach(testObjects).exec(function(err, obj) {
  ...
  assert(! err, "Received error " + util.inspect(err, {depth: null}));
    ...
    var value = V2Education.allScope().exec(function (err, results){
      assert(!err, "Received error " + util.inspect(err, {depth: null}));
    ...
    var value = V2Education.exactMatchScope().exec(function (err, results){
      assert(!err, "Received error " + util.inspect(err, {depth: null}));
    ...
    var value = V2Education.countScope().exec(function (err, results){
      assert(!err, "Received error " + util.inspect(err, {depth: null}));
    ...
    var value = V2Education.countExactMatchScope().exec(function (err, results){
      assert(!err, "Received error " + util.inspect(err, {depth: null}));
    ...
    var value = V2Education.sortedByNameScope().exec(function (err, results){
      assert(!err, "Received error " + util.inspect(err, {depth: null}));
