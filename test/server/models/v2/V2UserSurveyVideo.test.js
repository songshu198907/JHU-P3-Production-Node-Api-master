...
describe('V2UserSurveyVideo Model', function() {

  afterEach(function() {
    V2UserSurveyVideo.destroy();
  });
  ...
  it('should successfully create an instance of V2UserSurveyVideo', function(done) {
    var beforeCount;

    V2UserSurveyVideo.count()
      .then(function(count) {
        beforeCount = count;
        return V2UserSurveyVideo.create({"isComplete":true,"sortOrder":8770,"userSurveyId":59958,"videoNumber":50608,"videoPosition":84895.0043723797,"videoSourceVersion":"Anthologizing pistachios","videoType":"Mouses testamentary"});
      })
      .then(function(obj) {
  ...
  it('should successfully read an instance of V2UserSurveyVideo', function(done) {

    var attributes = {"isComplete":true,"sortOrder":14925,"userSurveyId":30943,"videoNumber":62777,"videoPosition":1976.530621633898,"videoSourceVersion":"Disengagements distinctiveness","videoType":"Frogman Medicares"};
    var id;

    if (attributes && Object.keys(attributes).length !== 0) {
      V2UserSurveyVideo.findOne(attributes)
        .then(function(record) {
          assert(!record, 'Should not have found a record.');
          return V2UserSurveyVideo.create(attributes);
        })
        .then(function(obj) {
          assert(obj, 'Failed to create object.');
          id = obj.id;
          return V2UserSurveyVideo.findOne({'id':obj.id});
        })
        .then(function(obj) {
  ...
  it('should successfully update an instance of V2UserSurveyVideo', function(done) {
    var attributes = {"isComplete":true,"sortOrder":9270,"userSurveyId":99034,"videoNumber":59731,"videoPosition":70958.45595931548,"videoSourceVersion":"Currycomb meetinghouses","videoType":"Threat alpaca"};
    var updatedObject;

    V2UserSurveyVideo.create(attributes)
      .then(function(obj) {
        assert(obj, 'Failed to create object.');
        var attributes = {"isComplete":false,"sortOrder":85405,"userSurveyId":48051,"videoNumber":28941,"videoPosition":20614.92656437396,"videoSourceVersion":"Linkage feudalistic","videoType":"Tupungato hallucination"};
        return V2UserSurveyVideo.update(obj.id, attributes);
      })
      .then(function(collection) {
        updatedObject = collection[0];
        assert(updatedObject, 'Failed to update object.');
        return V2UserSurveyVideo.findOne({'id':updatedObject.id});
      })
      .then(function(obj) {
  ...
  it('should successfully destroy an instance of V2UserSurveyVideo', function(done) {

    var attributes = {"isComplete":true,"sortOrder":34488,"userSurveyId":34003,"videoNumber":16193,"videoPosition":22821.35552135563,"videoSourceVersion":"Backfires mercury","videoType":"Halfpence folksy"};
    ...
    if (attributes && Object.keys(attributes).length !== 0) {
      V2UserSurveyVideo.findOne(attributes)
        ...
        .then(function(obj) {
          assert(obj, 'Failed to create object.');
          return V2UserSurveyVideo.destroy(obj.id);
        })
        .then(function(collection) {
  ...
  describe('custom callbacks', function() {
    it('should have a callbacks object', function() {
      assert(V2UserSurveyVideo.callbacks, 'V2UserSurveyVideo is missing the callbacks object');
    });
      ...
      it('should contain "beforeCreate" callback', function() {
        assert(V2UserSurveyVideo.callbacks['beforeCreate'], 'beforeCreate is missing.');
      });

      it('"beforeCreate" callback should be a function', function() {
      assert(_.isFunction(V2UserSurveyVideo.callbacks['beforeCreate']), 'beforeCreate should be a function.');
    });
    
      

      it('should contain "afterCreate" callback', function() {
        assert(V2UserSurveyVideo.callbacks['afterCreate'], 'afterCreate is missing.');
      });

      it('"afterCreate" callback should be a function', function() {
      assert(_.isFunction(V2UserSurveyVideo.callbacks['afterCreate']), 'afterCreate should be a function.');
    });
    
      

      it('should contain "beforeFind" callback', function() {
        assert(V2UserSurveyVideo.callbacks['beforeFind'], 'beforeFind is missing.');
      });

      it('"beforeFind" callback should be a function', function() {
      assert(_.isFunction(V2UserSurveyVideo.callbacks['beforeFind']), 'beforeFind should be a function.');
    });
    
      

      it('should contain "afterFind" callback', function() {
        assert(V2UserSurveyVideo.callbacks['afterFind'], 'afterFind is missing.');
      });

      it('"afterFind" callback should be a function', function() {
      assert(_.isFunction(V2UserSurveyVideo.callbacks['afterFind']), 'afterFind should be a function.');
    });
    
      

      it('should contain "beforeUpdate" callback', function() {
        assert(V2UserSurveyVideo.callbacks['beforeUpdate'], 'beforeUpdate is missing.');
      });

      it('"beforeUpdate" callback should be a function', function() {
      assert(_.isFunction(V2UserSurveyVideo.callbacks['beforeUpdate']), 'beforeUpdate should be a function.');
    });
    
      

      it('should contain "afterUpdate" callback', function() {
        assert(V2UserSurveyVideo.callbacks['afterUpdate'], 'afterUpdate is missing.');
      });

      it('"afterUpdate" callback should be a function', function() {
      assert(_.isFunction(V2UserSurveyVideo.callbacks['afterUpdate']), 'afterUpdate should be a function.');
    });
    
      

      it('should contain "beforeDestroy" callback', function() {
        assert(V2UserSurveyVideo.callbacks['beforeDestroy'], 'beforeDestroy is missing.');
      });

      it('"beforeDestroy" callback should be a function', function() {
      assert(_.isFunction(V2UserSurveyVideo.callbacks['beforeDestroy']), 'beforeDestroy should be a function.');
    });
    
      

      it('should contain "afterDestroy" callback', function() {
        assert(V2UserSurveyVideo.callbacks['afterDestroy'], 'afterDestroy is missing.');
      });

      it('"afterDestroy" callback should be a function', function() {
      assert(_.isFunction(V2UserSurveyVideo.callbacks['afterDestroy']), 'afterDestroy should be a function.');
    });
