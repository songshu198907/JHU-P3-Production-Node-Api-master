...
describe('V2JobAuditLogging Model', function() {

  afterEach(function() {
    V2JobAuditLogging.destroy();
  });
  ...
  it('should successfully create an instance of V2JobAuditLogging', function(done) {
    var beforeCount;

    V2JobAuditLogging.count()
      .then(function(count) {
        beforeCount = count;
        return V2JobAuditLogging.create({"createdAt":"2012-08-26 09:13:15 +0000","name":"Lino Goodwin","rowsEffected":"Renaissances physiology"});
      })
      .then(function(obj) {
        assert(obj, 'Failed to create object.');
        return V2JobAuditLogging.count();
      })
      .then(function(count) {
  ...
  it('should successfully read an instance of V2JobAuditLogging', function(done) {

    var attributes = {"createdAt":"1986-07-20 16:29:56 +0000","name":"Elmira Christiansen","rowsEffected":"Arabs damage"};
    var id;

    if (attributes && Object.keys(attributes).length !== 0) {
      V2JobAuditLogging.findOne(attributes)
        .then(function(record) {
          assert(!record, 'Should not have found a record.');
          return V2JobAuditLogging.create(attributes);
        })
        .then(function(obj) {
          assert(obj, 'Failed to create object.');
          id = obj.id;
          return V2JobAuditLogging.findOne({'id':obj.id});
        })
        .then(function(obj) {
  ...
  it('should successfully update an instance of V2JobAuditLogging', function(done) {
    var attributes = {"createdAt":"1981-03-10 01:48:20 +0000","name":"Elvina Cruickshank","rowsEffected":"Associations commercializes"};
    var updatedObject;

    V2JobAuditLogging.create(attributes)
      .then(function(obj) {
        assert(obj, 'Failed to create object.');
        var attributes = {"createdAt":"1982-04-21 18:34:10 +0000","name":"Elaina Gorczany","rowsEffected":"Flies insensitivity"};
        return V2JobAuditLogging.update(obj.id, attributes);
      })
      .then(function(collection) {
        updatedObject = collection[0];
        assert(updatedObject, 'Failed to update object.');
        return V2JobAuditLogging.findOne({'id':updatedObject.id});
      })
      .then(function(obj) {
  ...
  it('should successfully destroy an instance of V2JobAuditLogging', function(done) {

    var attributes = {"createdAt":"1972-02-15 07:51:05 +0000","name":"Kristeen Metz","rowsEffected":"Lusting ratification"};
    ...
    if (attributes && Object.keys(attributes).length !== 0) {
      V2JobAuditLogging.findOne(attributes)
        ...
        .then(function(obj) {
          assert(obj, 'Failed to create object.');
          return V2JobAuditLogging.destroy(obj.id);
        })
        .then(function(collection) {
  ...
  describe('custom callbacks', function() {
    it('should have a callbacks object', function() {
      assert(V2JobAuditLogging.callbacks, 'V2JobAuditLogging is missing the callbacks object');
    });
      ...
      it('should contain "beforeCreate" callback', function() {
        assert(V2JobAuditLogging.callbacks['beforeCreate'], 'beforeCreate is missing.');
      });

      it('"beforeCreate" callback should be a function', function() {
      assert(_.isFunction(V2JobAuditLogging.callbacks['beforeCreate']), 'beforeCreate should be a function.');
    });
    
      

      it('should contain "afterCreate" callback', function() {
        assert(V2JobAuditLogging.callbacks['afterCreate'], 'afterCreate is missing.');
      });

      it('"afterCreate" callback should be a function', function() {
      assert(_.isFunction(V2JobAuditLogging.callbacks['afterCreate']), 'afterCreate should be a function.');
    });
    
      

      it('should contain "beforeFind" callback', function() {
        assert(V2JobAuditLogging.callbacks['beforeFind'], 'beforeFind is missing.');
      });

      it('"beforeFind" callback should be a function', function() {
      assert(_.isFunction(V2JobAuditLogging.callbacks['beforeFind']), 'beforeFind should be a function.');
    });
    
      

      it('should contain "afterFind" callback', function() {
        assert(V2JobAuditLogging.callbacks['afterFind'], 'afterFind is missing.');
      });

      it('"afterFind" callback should be a function', function() {
      assert(_.isFunction(V2JobAuditLogging.callbacks['afterFind']), 'afterFind should be a function.');
    });
    
      

      it('should contain "beforeUpdate" callback', function() {
        assert(V2JobAuditLogging.callbacks['beforeUpdate'], 'beforeUpdate is missing.');
      });

      it('"beforeUpdate" callback should be a function', function() {
      assert(_.isFunction(V2JobAuditLogging.callbacks['beforeUpdate']), 'beforeUpdate should be a function.');
    });
    
      

      it('should contain "afterUpdate" callback', function() {
        assert(V2JobAuditLogging.callbacks['afterUpdate'], 'afterUpdate is missing.');
      });

      it('"afterUpdate" callback should be a function', function() {
      assert(_.isFunction(V2JobAuditLogging.callbacks['afterUpdate']), 'afterUpdate should be a function.');
    });
    
      

      it('should contain "beforeDestroy" callback', function() {
        assert(V2JobAuditLogging.callbacks['beforeDestroy'], 'beforeDestroy is missing.');
      });

      it('"beforeDestroy" callback should be a function', function() {
      assert(_.isFunction(V2JobAuditLogging.callbacks['beforeDestroy']), 'beforeDestroy should be a function.');
    });
    
      

      it('should contain "afterDestroy" callback', function() {
        assert(V2JobAuditLogging.callbacks['afterDestroy'], 'afterDestroy is missing.');
      });

      it('"afterDestroy" callback should be a function', function() {
      assert(_.isFunction(V2JobAuditLogging.callbacks['afterDestroy']), 'afterDestroy should be a function.');
    });
