...
describe('V2VaccinationReminder Model', function() {

  afterEach(function() {
    V2VaccinationReminder.destroy();
  });
  ...
  it('should successfully create an instance of V2VaccinationReminder', function(done) {
    var beforeCount;

    V2VaccinationReminder.count()
      .then(function(count) {
        beforeCount = count;
        return V2VaccinationReminder.create({"messageText":"Emporiums swiped","triggerDaysFromDob":1627});
      })
      .then(function(obj) {
  ...
  it('should successfully read an instance of V2VaccinationReminder', function(done) {

    var attributes = {"messageText":"Prudent turboprops","triggerDaysFromDob":72322};
    var id;

    if (attributes && Object.keys(attributes).length !== 0) {
      V2VaccinationReminder.findOne(attributes)
        .then(function(record) {
          assert(!record, 'Should not have found a record.');
          return V2VaccinationReminder.create(attributes);
        })
        .then(function(obj) {
          assert(obj, 'Failed to create object.');
          id = obj.id;
          return V2VaccinationReminder.findOne({'id':obj.id});
        })
        .then(function(obj) {
  ...
  it('should successfully update an instance of V2VaccinationReminder', function(done) {
    var attributes = {"messageText":"Aggrandizement weaponless","triggerDaysFromDob":9130};
    var updatedObject;

    V2VaccinationReminder.create(attributes)
      .then(function(obj) {
        assert(obj, 'Failed to create object.');
        var attributes = {"messageText":"Reappraisals crystallization","triggerDaysFromDob":78188};
        return V2VaccinationReminder.update(obj.id, attributes);
      })
      .then(function(collection) {
        updatedObject = collection[0];
        assert(updatedObject, 'Failed to update object.');
        return V2VaccinationReminder.findOne({'id':updatedObject.id});
      })
      .then(function(obj) {
  ...
  it('should successfully destroy an instance of V2VaccinationReminder', function(done) {

    var attributes = {"messageText":"Grant courts","triggerDaysFromDob":74774};
    ...
    if (attributes && Object.keys(attributes).length !== 0) {
      V2VaccinationReminder.findOne(attributes)
        ...
        .then(function(obj) {
          assert(obj, 'Failed to create object.');
          return V2VaccinationReminder.destroy(obj.id);
        })
        .then(function(collection) {
  ...
  describe('custom callbacks', function() {
    it('should have a callbacks object', function() {
      assert(V2VaccinationReminder.callbacks, 'V2VaccinationReminder is missing the callbacks object');
    });
      ...
      it('should contain "beforeCreate" callback', function() {
        assert(V2VaccinationReminder.callbacks['beforeCreate'], 'beforeCreate is missing.');
      });

      it('"beforeCreate" callback should be a function', function() {
      assert(_.isFunction(V2VaccinationReminder.callbacks['beforeCreate']), 'beforeCreate should be a function.');
    });
    
      

      it('should contain "afterCreate" callback', function() {
        assert(V2VaccinationReminder.callbacks['afterCreate'], 'afterCreate is missing.');
      });

      it('"afterCreate" callback should be a function', function() {
      assert(_.isFunction(V2VaccinationReminder.callbacks['afterCreate']), 'afterCreate should be a function.');
    });
    
      

      it('should contain "beforeFind" callback', function() {
        assert(V2VaccinationReminder.callbacks['beforeFind'], 'beforeFind is missing.');
      });

      it('"beforeFind" callback should be a function', function() {
      assert(_.isFunction(V2VaccinationReminder.callbacks['beforeFind']), 'beforeFind should be a function.');
    });
    
      

      it('should contain "afterFind" callback', function() {
        assert(V2VaccinationReminder.callbacks['afterFind'], 'afterFind is missing.');
      });

      it('"afterFind" callback should be a function', function() {
      assert(_.isFunction(V2VaccinationReminder.callbacks['afterFind']), 'afterFind should be a function.');
    });
    
      

      it('should contain "beforeUpdate" callback', function() {
        assert(V2VaccinationReminder.callbacks['beforeUpdate'], 'beforeUpdate is missing.');
      });

      it('"beforeUpdate" callback should be a function', function() {
      assert(_.isFunction(V2VaccinationReminder.callbacks['beforeUpdate']), 'beforeUpdate should be a function.');
    });
    
      

      it('should contain "afterUpdate" callback', function() {
        assert(V2VaccinationReminder.callbacks['afterUpdate'], 'afterUpdate is missing.');
      });

      it('"afterUpdate" callback should be a function', function() {
      assert(_.isFunction(V2VaccinationReminder.callbacks['afterUpdate']), 'afterUpdate should be a function.');
    });
    
      

      it('should contain "beforeDestroy" callback', function() {
        assert(V2VaccinationReminder.callbacks['beforeDestroy'], 'beforeDestroy is missing.');
      });

      it('"beforeDestroy" callback should be a function', function() {
      assert(_.isFunction(V2VaccinationReminder.callbacks['beforeDestroy']), 'beforeDestroy should be a function.');
    });
    
      

      it('should contain "afterDestroy" callback', function() {
        assert(V2VaccinationReminder.callbacks['afterDestroy'], 'afterDestroy is missing.');
      });

      it('"afterDestroy" callback should be a function', function() {
      assert(_.isFunction(V2VaccinationReminder.callbacks['afterDestroy']), 'afterDestroy should be a function.');
    });
