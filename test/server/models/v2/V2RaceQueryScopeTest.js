...
describe('V2Race query scopes', function() {

  beforeEach( function(done){
	objects = [];
var testObjects = [];
  testObjects.push({"name":"Inger Hintz"});
  testObjects.push({"name":"Mitchell Smith"});
  testObjects.push({"name":"Tyron Fadel"});
...
V2Race.createEach(testObjects).exec(function(err, obj) {
  ...
  assert(! err, "Received error " + util.inspect(err, {depth: null}));
    ...
    var value = V2Race.allScope().exec(function (err, results){
      assert(!err, "Received error " + util.inspect(err, {depth: null}));
    ...
    var value = V2Race.exactMatchScope().exec(function (err, results){
      assert(!err, "Received error " + util.inspect(err, {depth: null}));
    ...
    var value = V2Race.countScope().exec(function (err, results){
      assert(!err, "Received error " + util.inspect(err, {depth: null}));
    ...
    var value = V2Race.countExactMatchScope().exec(function (err, results){
      assert(!err, "Received error " + util.inspect(err, {depth: null}));
    ...
    var value = V2Race.sortedByNameScope().exec(function (err, results){
      assert(!err, "Received error " + util.inspect(err, {depth: null}));
