...
describe('V2Survey Model', function() {
  ...
  it('should successfully create an instance of V2Survey', function(done) {
    var beforeCount;

    V2Survey.count()
      .then(function(count) {
        beforeCount = count;
        return V2Survey.create({"interventionGroup":"Conflicts anthropologist","isActive":true,"name":"Bryan Wolff","patientType":19560,"surveyType":"Subcontractors helices","totalQuestions":63530,"version":98234});
      })
      .then(function(obj) {
        assert(obj, 'Failed to create object.');
        return V2Survey.count();
      })
      .then(function(count) {
  ...
  it('should successfully read an instance of V2Survey', function(done) {

    var attributes = {"interventionGroup":"Indiscreetly interconnecting","isActive":false,"name":"Fernando Stanton","patientType":45743,"surveyType":"Blunderbusses loafers","totalQuestions":9119,"version":72248};
    var id;

    if (attributes && Object.keys(attributes).length !== 0) {
      V2Survey.findOne(attributes)
        .then(function(record) {
          assert(!record, 'Should not have found a record.');
          return V2Survey.create(attributes);
        })
        .then(function(obj) {
          assert(obj, 'Failed to create object.');
          id = obj.id;
          return V2Survey.findOne({'id':obj.id});
        })
        .then(function(obj) {
  ...
  it('should successfully update an instance of V2Survey', function(done) {
    var attributes = {"interventionGroup":"Reverberations circumscribed","isActive":false,"name":"Prudence Predovic","patientType":54854,"surveyType":"Evangelists instrumentality","totalQuestions":84705,"version":985};
    var updatedObject;

    V2Survey.create(attributes)
      .then(function(obj) {
        assert(obj, 'Failed to create object.');
        var attributes = {"interventionGroup":"Windpipe soliloquy","isActive":false,"name":"Andreas Koelpin","patientType":93010,"surveyType":"Misrepresenting Ebola","totalQuestions":80727,"version":23448};
        return V2Survey.update(obj.id, attributes);
      })
      .then(function(collection) {
        updatedObject = collection[0];
        assert(updatedObject, 'Failed to update object.');
        return V2Survey.findOne({'id':updatedObject.id});
      })
      .then(function(obj) {
  ...
  it('should successfully destroy an instance of V2Survey', function(done) {

    var attributes = {"interventionGroup":"Bluetooth Nisei","isActive":true,"name":"Jacquelyn Upton","patientType":23122,"surveyType":"Famishes beamed","totalQuestions":35865,"version":67694};
    ...
    if (attributes && Object.keys(attributes).length !== 0) {
      V2Survey.findOne(attributes)
        ...
        .then(function(obj) {
          assert(obj, 'Failed to create object.');
          return V2Survey.destroy(obj.id);
        })
        .then(function(collection) {
  ...
  describe('custom callbacks', function() {
    it('should have a callbacks object', function() {
      assert(V2Survey.callbacks, 'V2Survey is missing the callbacks object');
    });
      ...
      it('should contain "beforeCreate" callback', function() {
        assert(V2Survey.callbacks['beforeCreate'], 'beforeCreate is missing.');
      });

      it('"beforeCreate" callback should be a function', function() {
      assert(_.isFunction(V2Survey.callbacks['beforeCreate']), 'beforeCreate should be a function.');
    });
    
      

      it('should contain "afterCreate" callback', function() {
        assert(V2Survey.callbacks['afterCreate'], 'afterCreate is missing.');
      });

      it('"afterCreate" callback should be a function', function() {
      assert(_.isFunction(V2Survey.callbacks['afterCreate']), 'afterCreate should be a function.');
    });
    
      

      it('should contain "beforeFind" callback', function() {
        assert(V2Survey.callbacks['beforeFind'], 'beforeFind is missing.');
      });

      it('"beforeFind" callback should be a function', function() {
      assert(_.isFunction(V2Survey.callbacks['beforeFind']), 'beforeFind should be a function.');
    });
    
      

      it('should contain "afterFind" callback', function() {
        assert(V2Survey.callbacks['afterFind'], 'afterFind is missing.');
      });

      it('"afterFind" callback should be a function', function() {
      assert(_.isFunction(V2Survey.callbacks['afterFind']), 'afterFind should be a function.');
    });
    
      

      it('should contain "beforeUpdate" callback', function() {
        assert(V2Survey.callbacks['beforeUpdate'], 'beforeUpdate is missing.');
      });

      it('"beforeUpdate" callback should be a function', function() {
      assert(_.isFunction(V2Survey.callbacks['beforeUpdate']), 'beforeUpdate should be a function.');
    });
    
      

      it('should contain "afterUpdate" callback', function() {
        assert(V2Survey.callbacks['afterUpdate'], 'afterUpdate is missing.');
      });

      it('"afterUpdate" callback should be a function', function() {
      assert(_.isFunction(V2Survey.callbacks['afterUpdate']), 'afterUpdate should be a function.');
    });
    
      

      it('should contain "beforeDestroy" callback', function() {
        assert(V2Survey.callbacks['beforeDestroy'], 'beforeDestroy is missing.');
      });

      it('"beforeDestroy" callback should be a function', function() {
      assert(_.isFunction(V2Survey.callbacks['beforeDestroy']), 'beforeDestroy should be a function.');
    });
    
      

      it('should contain "afterDestroy" callback', function() {
        assert(V2Survey.callbacks['afterDestroy'], 'afterDestroy is missing.');
      });

      it('"afterDestroy" callback should be a function', function() {
      assert(_.isFunction(V2Survey.callbacks['afterDestroy']), 'afterDestroy should be a function.');
    });
