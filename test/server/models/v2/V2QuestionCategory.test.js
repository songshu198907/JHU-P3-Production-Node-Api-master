...
describe('V2QuestionCategory Model', function() {

  afterEach(function() {
    V2QuestionCategory.destroy();
  });
  ...
  it('should successfully create an instance of V2QuestionCategory', function(done) {
    var beforeCount;

    V2QuestionCategory.count()
      .then(function(count) {
        beforeCount = count;
        return V2QuestionCategory.create({"name":"Lindsey Davis"});
      })
      .then(function(obj) {
  ...
  it('should successfully read an instance of V2QuestionCategory', function(done) {

    var attributes = {"name":"Sharleen Jacobson"};
    var id;

    if (attributes && Object.keys(attributes).length !== 0) {
      V2QuestionCategory.findOne(attributes)
        .then(function(record) {
          assert(!record, 'Should not have found a record.');
          return V2QuestionCategory.create(attributes);
        })
        .then(function(obj) {
          assert(obj, 'Failed to create object.');
          id = obj.id;
          return V2QuestionCategory.findOne({'id':obj.id});
        })
        .then(function(obj) {
  ...
  it('should successfully update an instance of V2QuestionCategory', function(done) {
    var attributes = {"name":"Kiara Kovacek"};
    var updatedObject;

    V2QuestionCategory.create(attributes)
      .then(function(obj) {
        assert(obj, 'Failed to create object.');
        var attributes = {"name":"Chan Fadel"};
        return V2QuestionCategory.update(obj.id, attributes);
      })
      .then(function(collection) {
        updatedObject = collection[0];
        assert(updatedObject, 'Failed to update object.');
        return V2QuestionCategory.findOne({'id':updatedObject.id});
      })
      .then(function(obj) {
  ...
  it('should successfully destroy an instance of V2QuestionCategory', function(done) {

    var attributes = {"name":"Jacquiline Okuneva"};
    ...
    if (attributes && Object.keys(attributes).length !== 0) {
      V2QuestionCategory.findOne(attributes)
        ...
        .then(function(obj) {
          assert(obj, 'Failed to create object.');
          return V2QuestionCategory.destroy(obj.id);
        })
        .then(function(collection) {
  ...
  describe('custom callbacks', function() {
    it('should have a callbacks object', function() {
      assert(V2QuestionCategory.callbacks, 'V2QuestionCategory is missing the callbacks object');
    });
      ...
      it('should contain "beforeCreate" callback', function() {
        assert(V2QuestionCategory.callbacks['beforeCreate'], 'beforeCreate is missing.');
      });

      it('"beforeCreate" callback should be a function', function() {
      assert(_.isFunction(V2QuestionCategory.callbacks['beforeCreate']), 'beforeCreate should be a function.');
    });
    
      

      it('should contain "afterCreate" callback', function() {
        assert(V2QuestionCategory.callbacks['afterCreate'], 'afterCreate is missing.');
      });

      it('"afterCreate" callback should be a function', function() {
      assert(_.isFunction(V2QuestionCategory.callbacks['afterCreate']), 'afterCreate should be a function.');
    });
    
      

      it('should contain "beforeFind" callback', function() {
        assert(V2QuestionCategory.callbacks['beforeFind'], 'beforeFind is missing.');
      });

      it('"beforeFind" callback should be a function', function() {
      assert(_.isFunction(V2QuestionCategory.callbacks['beforeFind']), 'beforeFind should be a function.');
    });
    
      

      it('should contain "afterFind" callback', function() {
        assert(V2QuestionCategory.callbacks['afterFind'], 'afterFind is missing.');
      });

      it('"afterFind" callback should be a function', function() {
      assert(_.isFunction(V2QuestionCategory.callbacks['afterFind']), 'afterFind should be a function.');
    });
    
      

      it('should contain "beforeUpdate" callback', function() {
        assert(V2QuestionCategory.callbacks['beforeUpdate'], 'beforeUpdate is missing.');
      });

      it('"beforeUpdate" callback should be a function', function() {
      assert(_.isFunction(V2QuestionCategory.callbacks['beforeUpdate']), 'beforeUpdate should be a function.');
    });
    
      

      it('should contain "afterUpdate" callback', function() {
        assert(V2QuestionCategory.callbacks['afterUpdate'], 'afterUpdate is missing.');
      });

      it('"afterUpdate" callback should be a function', function() {
      assert(_.isFunction(V2QuestionCategory.callbacks['afterUpdate']), 'afterUpdate should be a function.');
    });
    
      

      it('should contain "beforeDestroy" callback', function() {
        assert(V2QuestionCategory.callbacks['beforeDestroy'], 'beforeDestroy is missing.');
      });

      it('"beforeDestroy" callback should be a function', function() {
      assert(_.isFunction(V2QuestionCategory.callbacks['beforeDestroy']), 'beforeDestroy should be a function.');
    });
    
      

      it('should contain "afterDestroy" callback', function() {
        assert(V2QuestionCategory.callbacks['afterDestroy'], 'afterDestroy is missing.');
      });

      it('"afterDestroy" callback should be a function', function() {
      assert(_.isFunction(V2QuestionCategory.callbacks['afterDestroy']), 'afterDestroy should be a function.');
    });
