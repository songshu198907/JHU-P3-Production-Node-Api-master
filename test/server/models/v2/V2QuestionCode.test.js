...
describe('V2QuestionCode Model', function() {

  afterEach(function() {
    V2QuestionCode.destroy();
  });
  ...
  it('should successfully create an instance of V2QuestionCode', function(done) {
    var beforeCount;

    V2QuestionCode.count()
      .then(function(count) {
        beforeCount = count;
        return V2QuestionCode.create({"name":"Josie Kautzer"});
      })
      .then(function(obj) {
  ...
  it('should successfully read an instance of V2QuestionCode', function(done) {

    var attributes = {"name":"Mitsuko Lakin"};
    var id;

    if (attributes && Object.keys(attributes).length !== 0) {
      V2QuestionCode.findOne(attributes)
        .then(function(record) {
          assert(!record, 'Should not have found a record.');
          return V2QuestionCode.create(attributes);
        })
        .then(function(obj) {
          assert(obj, 'Failed to create object.');
          id = obj.id;
          return V2QuestionCode.findOne({'id':obj.id});
        })
        .then(function(obj) {
  ...
  it('should successfully update an instance of V2QuestionCode', function(done) {
    var attributes = {"name":"Joycelyn Spinka"};
    var updatedObject;

    V2QuestionCode.create(attributes)
      .then(function(obj) {
        assert(obj, 'Failed to create object.');
        var attributes = {"name":"Mariana Bruen"};
        return V2QuestionCode.update(obj.id, attributes);
      })
      .then(function(collection) {
        updatedObject = collection[0];
        assert(updatedObject, 'Failed to update object.');
        return V2QuestionCode.findOne({'id':updatedObject.id});
      })
      .then(function(obj) {
  ...
  it('should successfully destroy an instance of V2QuestionCode', function(done) {

    var attributes = {"name":"Ellamae Jewess"};
    ...
    if (attributes && Object.keys(attributes).length !== 0) {
      V2QuestionCode.findOne(attributes)
        ...
        .then(function(obj) {
          assert(obj, 'Failed to create object.');
          return V2QuestionCode.destroy(obj.id);
        })
        .then(function(collection) {
  ...
  describe('custom callbacks', function() {
    it('should have a callbacks object', function() {
      assert(V2QuestionCode.callbacks, 'V2QuestionCode is missing the callbacks object');
    });
      ...
      it('should contain "beforeCreate" callback', function() {
        assert(V2QuestionCode.callbacks['beforeCreate'], 'beforeCreate is missing.');
      });

      it('"beforeCreate" callback should be a function', function() {
      assert(_.isFunction(V2QuestionCode.callbacks['beforeCreate']), 'beforeCreate should be a function.');
    });
    
      

      it('should contain "afterCreate" callback', function() {
        assert(V2QuestionCode.callbacks['afterCreate'], 'afterCreate is missing.');
      });

      it('"afterCreate" callback should be a function', function() {
      assert(_.isFunction(V2QuestionCode.callbacks['afterCreate']), 'afterCreate should be a function.');
    });
    
      

      it('should contain "beforeFind" callback', function() {
        assert(V2QuestionCode.callbacks['beforeFind'], 'beforeFind is missing.');
      });

      it('"beforeFind" callback should be a function', function() {
      assert(_.isFunction(V2QuestionCode.callbacks['beforeFind']), 'beforeFind should be a function.');
    });
    
      

      it('should contain "afterFind" callback', function() {
        assert(V2QuestionCode.callbacks['afterFind'], 'afterFind is missing.');
      });

      it('"afterFind" callback should be a function', function() {
      assert(_.isFunction(V2QuestionCode.callbacks['afterFind']), 'afterFind should be a function.');
    });
    
      

      it('should contain "beforeUpdate" callback', function() {
        assert(V2QuestionCode.callbacks['beforeUpdate'], 'beforeUpdate is missing.');
      });

      it('"beforeUpdate" callback should be a function', function() {
      assert(_.isFunction(V2QuestionCode.callbacks['beforeUpdate']), 'beforeUpdate should be a function.');
    });
    
      

      it('should contain "afterUpdate" callback', function() {
        assert(V2QuestionCode.callbacks['afterUpdate'], 'afterUpdate is missing.');
      });

      it('"afterUpdate" callback should be a function', function() {
      assert(_.isFunction(V2QuestionCode.callbacks['afterUpdate']), 'afterUpdate should be a function.');
    });
    
      

      it('should contain "beforeDestroy" callback', function() {
        assert(V2QuestionCode.callbacks['beforeDestroy'], 'beforeDestroy is missing.');
      });

      it('"beforeDestroy" callback should be a function', function() {
      assert(_.isFunction(V2QuestionCode.callbacks['beforeDestroy']), 'beforeDestroy should be a function.');
    });
    
      

      it('should contain "afterDestroy" callback', function() {
        assert(V2QuestionCode.callbacks['afterDestroy'], 'afterDestroy is missing.');
      });

      it('"afterDestroy" callback should be a function', function() {
      assert(_.isFunction(V2QuestionCode.callbacks['afterDestroy']), 'afterDestroy should be a function.');
    });
