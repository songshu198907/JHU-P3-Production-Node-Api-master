...
describe('V2VideoAuditLogging Model', function() {

  afterEach(function() {
    V2VideoAuditLogging.destroy();
  });
  ...
  it('should successfully create an instance of V2VideoAuditLogging', function(done) {
    var beforeCount;

    V2VideoAuditLogging.count()
      .then(function(count) {
        beforeCount = count;
        return V2VideoAuditLogging.create({"createdAt":"1997-11-05 23:15:44 +0000","duration":86344.05205406348,"userId":42509,"videoId":63330,"watchedEntireVideo":false});
      })
      .then(function(obj) {
  ...
  it('should successfully read an instance of V2VideoAuditLogging', function(done) {

    var attributes = {"createdAt":"2015-01-24 07:04:00 +0000","duration":34498.372795477975,"userId":67797,"videoId":87958,"watchedEntireVideo":true};
    var id;

    if (attributes && Object.keys(attributes).length !== 0) {
      V2VideoAuditLogging.findOne(attributes)
        .then(function(record) {
          assert(!record, 'Should not have found a record.');
          return V2VideoAuditLogging.create(attributes);
        })
        .then(function(obj) {
          assert(obj, 'Failed to create object.');
          id = obj.id;
          return V2VideoAuditLogging.findOne({'id':obj.id});
        })
        .then(function(obj) {
  ...
  it('should successfully update an instance of V2VideoAuditLogging', function(done) {
    var attributes = {"createdAt":"1995-11-13 06:44:39 +0000","duration":35054.543611482375,"userId":99758,"videoId":73053,"watchedEntireVideo":true};
    var updatedObject;

    V2VideoAuditLogging.create(attributes)
      .then(function(obj) {
        assert(obj, 'Failed to create object.');
        var attributes = {"createdAt":"1987-07-27 20:23:21 +0000","duration":21716.96022160894,"userId":78322,"videoId":47228,"watchedEntireVideo":false};
        return V2VideoAuditLogging.update(obj.id, attributes);
      })
      .then(function(collection) {
        updatedObject = collection[0];
        assert(updatedObject, 'Failed to update object.');
        return V2VideoAuditLogging.findOne({'id':updatedObject.id});
      })
      .then(function(obj) {
  ...
  it('should successfully destroy an instance of V2VideoAuditLogging', function(done) {

    var attributes = {"createdAt":"2004-02-27 05:32:55 +0000","duration":52298.667095978155,"userId":63988,"videoId":74291,"watchedEntireVideo":false};
    ...
    if (attributes && Object.keys(attributes).length !== 0) {
      V2VideoAuditLogging.findOne(attributes)
        ...
        .then(function(obj) {
          assert(obj, 'Failed to create object.');
          return V2VideoAuditLogging.destroy(obj.id);
        })
        .then(function(collection) {
  ...
  describe('custom callbacks', function() {
    it('should have a callbacks object', function() {
      assert(V2VideoAuditLogging.callbacks, 'V2VideoAuditLogging is missing the callbacks object');
    });
      ...
      it('should contain "beforeCreate" callback', function() {
        assert(V2VideoAuditLogging.callbacks['beforeCreate'], 'beforeCreate is missing.');
      });

      it('"beforeCreate" callback should be a function', function() {
      assert(_.isFunction(V2VideoAuditLogging.callbacks['beforeCreate']), 'beforeCreate should be a function.');
    });
    
      

      it('should contain "afterCreate" callback', function() {
        assert(V2VideoAuditLogging.callbacks['afterCreate'], 'afterCreate is missing.');
      });

      it('"afterCreate" callback should be a function', function() {
      assert(_.isFunction(V2VideoAuditLogging.callbacks['afterCreate']), 'afterCreate should be a function.');
    });
    
      

      it('should contain "beforeFind" callback', function() {
        assert(V2VideoAuditLogging.callbacks['beforeFind'], 'beforeFind is missing.');
      });

      it('"beforeFind" callback should be a function', function() {
      assert(_.isFunction(V2VideoAuditLogging.callbacks['beforeFind']), 'beforeFind should be a function.');
    });
    
      

      it('should contain "afterFind" callback', function() {
        assert(V2VideoAuditLogging.callbacks['afterFind'], 'afterFind is missing.');
      });

      it('"afterFind" callback should be a function', function() {
      assert(_.isFunction(V2VideoAuditLogging.callbacks['afterFind']), 'afterFind should be a function.');
    });
    
      

      it('should contain "beforeUpdate" callback', function() {
        assert(V2VideoAuditLogging.callbacks['beforeUpdate'], 'beforeUpdate is missing.');
      });

      it('"beforeUpdate" callback should be a function', function() {
      assert(_.isFunction(V2VideoAuditLogging.callbacks['beforeUpdate']), 'beforeUpdate should be a function.');
    });
    
      

      it('should contain "afterUpdate" callback', function() {
        assert(V2VideoAuditLogging.callbacks['afterUpdate'], 'afterUpdate is missing.');
      });

      it('"afterUpdate" callback should be a function', function() {
      assert(_.isFunction(V2VideoAuditLogging.callbacks['afterUpdate']), 'afterUpdate should be a function.');
    });
    
      

      it('should contain "beforeDestroy" callback', function() {
        assert(V2VideoAuditLogging.callbacks['beforeDestroy'], 'beforeDestroy is missing.');
      });

      it('"beforeDestroy" callback should be a function', function() {
      assert(_.isFunction(V2VideoAuditLogging.callbacks['beforeDestroy']), 'beforeDestroy should be a function.');
    });
    
      

      it('should contain "afterDestroy" callback', function() {
        assert(V2VideoAuditLogging.callbacks['afterDestroy'], 'afterDestroy is missing.');
      });

      it('"afterDestroy" callback should be a function', function() {
      assert(_.isFunction(V2VideoAuditLogging.callbacks['afterDestroy']), 'afterDestroy should be a function.');
    });
