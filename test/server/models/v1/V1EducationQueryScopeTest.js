...
describe('V1Education query scopes', function() {

  beforeEach( function(done){
	objects = [];
var testObjects = [];
  testObjects.push({"name":"Tory Lockman"});
  testObjects.push({"name":"Rolf Kemmer"});
  testObjects.push({"name":"Georgene Effertz"});
...
V1Education.createEach(testObjects).exec(function(err, obj) {
  ...
  assert(! err, "Received error " + util.inspect(err, {depth: null}));
    ...
    var value = V1Education.allScope().exec(function (err, results){
      assert(!err, "Received error " + util.inspect(err, {depth: null}));
    ...
    var value = V1Education.exactMatchScope().exec(function (err, results){
      assert(!err, "Received error " + util.inspect(err, {depth: null}));
    ...
    var value = V1Education.countScope().exec(function (err, results){
      assert(!err, "Received error " + util.inspect(err, {depth: null}));
    ...
    var value = V1Education.countExactMatchScope().exec(function (err, results){
      assert(!err, "Received error " + util.inspect(err, {depth: null}));
    ...
    var value = V1Education.sortedByNameScope().exec(function (err, results){
      assert(!err, "Received error " + util.inspect(err, {depth: null}));
