...
describe('V1QuestionType Model', function() {

  afterEach(function() {
    V1QuestionType.destroy();
  });
  ...
  it('should successfully create an instance of V1QuestionType', function(done) {
    var beforeCount;

    V1QuestionType.count()
      .then(function(count) {
        beforeCount = count;
        return V1QuestionType.create({"controlType":"Unrelentingly overspecializes","name":"Todd Emard"});
      })
      .then(function(obj) {
        assert(obj, 'Failed to create object.');
        return V1QuestionType.count();
      })
      .then(function(count) {
  ...
  it('should successfully read an instance of V1QuestionType', function(done) {

    var attributes = {"controlType":"Handful misappropriate","name":"Sunshine Morar"};
    var id;

    if (attributes && Object.keys(attributes).length !== 0) {
      V1QuestionType.findOne(attributes)
        .then(function(record) {
          assert(!record, 'Should not have found a record.');
          return V1QuestionType.create(attributes);
        })
        .then(function(obj) {
          assert(obj, 'Failed to create object.');
          id = obj.id;
          return V1QuestionType.findOne({'id':obj.id});
        })
        .then(function(obj) {
  ...
  it('should successfully update an instance of V1QuestionType', function(done) {
    var attributes = {"controlType":"Blanches Casals","name":"Jack Hyatt"};
    var updatedObject;

    V1QuestionType.create(attributes)
      .then(function(obj) {
        assert(obj, 'Failed to create object.');
        var attributes = {"controlType":"Bandolier arrogant","name":"Elwood Reinger"};
        return V1QuestionType.update(obj.id, attributes);
      })
      .then(function(collection) {
        updatedObject = collection[0];
        assert(updatedObject, 'Failed to update object.');
        return V1QuestionType.findOne({'id':updatedObject.id});
      })
      .then(function(obj) {
  ...
  it('should successfully destroy an instance of V1QuestionType', function(done) {

    var attributes = {"controlType":"Monrovia flops","name":"Ivonne Jones"};
    ...
    if (attributes && Object.keys(attributes).length !== 0) {
      V1QuestionType.findOne(attributes)
        ...
        .then(function(obj) {
          assert(obj, 'Failed to create object.');
          return V1QuestionType.destroy(obj.id);
        })
        .then(function(collection) {
  ...
  describe('custom callbacks', function() {
    it('should have a callbacks object', function() {
      assert(V1QuestionType.callbacks, 'V1QuestionType is missing the callbacks object');
    });
      ...
      it('should contain "beforeCreate" callback', function() {
        assert(V1QuestionType.callbacks['beforeCreate'], 'beforeCreate is missing.');
      });

      it('"beforeCreate" callback should be a function', function() {
      assert(_.isFunction(V1QuestionType.callbacks['beforeCreate']), 'beforeCreate should be a function.');
    });
    
      

      it('should contain "afterCreate" callback', function() {
        assert(V1QuestionType.callbacks['afterCreate'], 'afterCreate is missing.');
      });

      it('"afterCreate" callback should be a function', function() {
      assert(_.isFunction(V1QuestionType.callbacks['afterCreate']), 'afterCreate should be a function.');
    });
    
      

      it('should contain "beforeFind" callback', function() {
        assert(V1QuestionType.callbacks['beforeFind'], 'beforeFind is missing.');
      });

      it('"beforeFind" callback should be a function', function() {
      assert(_.isFunction(V1QuestionType.callbacks['beforeFind']), 'beforeFind should be a function.');
    });
    
      

      it('should contain "afterFind" callback', function() {
        assert(V1QuestionType.callbacks['afterFind'], 'afterFind is missing.');
      });

      it('"afterFind" callback should be a function', function() {
      assert(_.isFunction(V1QuestionType.callbacks['afterFind']), 'afterFind should be a function.');
    });
    
      

      it('should contain "beforeUpdate" callback', function() {
        assert(V1QuestionType.callbacks['beforeUpdate'], 'beforeUpdate is missing.');
      });

      it('"beforeUpdate" callback should be a function', function() {
      assert(_.isFunction(V1QuestionType.callbacks['beforeUpdate']), 'beforeUpdate should be a function.');
    });
    
      

      it('should contain "afterUpdate" callback', function() {
        assert(V1QuestionType.callbacks['afterUpdate'], 'afterUpdate is missing.');
      });

      it('"afterUpdate" callback should be a function', function() {
      assert(_.isFunction(V1QuestionType.callbacks['afterUpdate']), 'afterUpdate should be a function.');
    });
    
      

      it('should contain "beforeDestroy" callback', function() {
        assert(V1QuestionType.callbacks['beforeDestroy'], 'beforeDestroy is missing.');
      });

      it('"beforeDestroy" callback should be a function', function() {
      assert(_.isFunction(V1QuestionType.callbacks['beforeDestroy']), 'beforeDestroy should be a function.');
    });
    
      

      it('should contain "afterDestroy" callback', function() {
        assert(V1QuestionType.callbacks['afterDestroy'], 'afterDestroy is missing.');
      });

      it('"afterDestroy" callback should be a function', function() {
      assert(_.isFunction(V1QuestionType.callbacks['afterDestroy']), 'afterDestroy should be a function.');
    });
