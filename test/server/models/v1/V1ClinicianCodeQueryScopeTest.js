...
describe('V1ClinicianCode query scopes', function() {

  beforeEach( function(done){
	objects = [];
var testObjects = [];
  testObjects.push({"code":"Plies flibbertigibbet","name":"Milagros Fisher"});
  testObjects.push({"code":"Circling evocative","name":"Shelton Hickle"});
  testObjects.push({"code":"Cyclones terminates","name":"Marquitta Lynch"});
...
V1ClinicianCode.createEach(testObjects).exec(function(err, obj) {
  ...
  assert(! err, "Received error " + util.inspect(err, {depth: null}));
    ...
    var value = V1ClinicianCode.allScope().exec(function (err, results){
      assert(!err, "Received error " + util.inspect(err, {depth: null}));
    ...
    var value = V1ClinicianCode.exactMatchScope().exec(function (err, results){
      assert(!err, "Received error " + util.inspect(err, {depth: null}));
    ...
    var value = V1ClinicianCode.countScope().exec(function (err, results){
      assert(!err, "Received error " + util.inspect(err, {depth: null}));
    ...
    var value = V1ClinicianCode.countExactMatchScope().exec(function (err, results){
      assert(!err, "Received error " + util.inspect(err, {depth: null}));
    ...
    var value = V1ClinicianCode.sortedByNameScope().exec(function (err, results){
      assert(!err, "Received error " + util.inspect(err, {depth: null}));
