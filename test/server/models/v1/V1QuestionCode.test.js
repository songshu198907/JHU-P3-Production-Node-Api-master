...
describe('V1QuestionCode Model', function() {

  afterEach(function() {
    V1QuestionCode.destroy();
  });
  ...
  it('should successfully create an instance of V1QuestionCode', function(done) {
    var beforeCount;

    V1QuestionCode.count()
      .then(function(count) {
        beforeCount = count;
        return V1QuestionCode.create({"name":"Louis Jakubowski"});
      })
      .then(function(obj) {
        assert(obj, 'Failed to create object.');
        return V1QuestionCode.count();
      })
      .then(function(count) {
  ...
  it('should successfully read an instance of V1QuestionCode', function(done) {

    var attributes = {"name":"Thaddeus O'Reilly"};
    var id;

    if (attributes && Object.keys(attributes).length !== 0) {
      V1QuestionCode.findOne(attributes)
        .then(function(record) {
          assert(!record, 'Should not have found a record.');
          return V1QuestionCode.create(attributes);
        })
        .then(function(obj) {
          assert(obj, 'Failed to create object.');
          id = obj.id;
          return V1QuestionCode.findOne({'id':obj.id});
        })
        .then(function(obj) {
  ...
  it('should successfully update an instance of V1QuestionCode', function(done) {
    var attributes = {"name":"Rolland Sporer"};
    var updatedObject;

    V1QuestionCode.create(attributes)
      .then(function(obj) {
        assert(obj, 'Failed to create object.');
        var attributes = {"name":"Angelo VonRueden"};
        return V1QuestionCode.update(obj.id, attributes);
      })
      .then(function(collection) {
        updatedObject = collection[0];
        assert(updatedObject, 'Failed to update object.');
        return V1QuestionCode.findOne({'id':updatedObject.id});
      })
      .then(function(obj) {
  ...
  it('should successfully destroy an instance of V1QuestionCode', function(done) {

    var attributes = {"name":"Nathan Mraz"};
    ...
    if (attributes && Object.keys(attributes).length !== 0) {
      V1QuestionCode.findOne(attributes)
        ...
        .then(function(obj) {
          assert(obj, 'Failed to create object.');
          return V1QuestionCode.destroy(obj.id);
        })
        .then(function(collection) {
  ...
  describe('custom callbacks', function() {
    it('should have a callbacks object', function() {
      assert(V1QuestionCode.callbacks, 'V1QuestionCode is missing the callbacks object');
    });
      ...
      it('should contain "beforeCreate" callback', function() {
        assert(V1QuestionCode.callbacks['beforeCreate'], 'beforeCreate is missing.');
      });

      it('"beforeCreate" callback should be a function', function() {
      assert(_.isFunction(V1QuestionCode.callbacks['beforeCreate']), 'beforeCreate should be a function.');
    });
    
      

      it('should contain "afterCreate" callback', function() {
        assert(V1QuestionCode.callbacks['afterCreate'], 'afterCreate is missing.');
      });

      it('"afterCreate" callback should be a function', function() {
      assert(_.isFunction(V1QuestionCode.callbacks['afterCreate']), 'afterCreate should be a function.');
    });
    
      

      it('should contain "beforeFind" callback', function() {
        assert(V1QuestionCode.callbacks['beforeFind'], 'beforeFind is missing.');
      });

      it('"beforeFind" callback should be a function', function() {
      assert(_.isFunction(V1QuestionCode.callbacks['beforeFind']), 'beforeFind should be a function.');
    });
    
      

      it('should contain "afterFind" callback', function() {
        assert(V1QuestionCode.callbacks['afterFind'], 'afterFind is missing.');
      });

      it('"afterFind" callback should be a function', function() {
      assert(_.isFunction(V1QuestionCode.callbacks['afterFind']), 'afterFind should be a function.');
    });
    
      

      it('should contain "beforeUpdate" callback', function() {
        assert(V1QuestionCode.callbacks['beforeUpdate'], 'beforeUpdate is missing.');
      });

      it('"beforeUpdate" callback should be a function', function() {
      assert(_.isFunction(V1QuestionCode.callbacks['beforeUpdate']), 'beforeUpdate should be a function.');
    });
    
      

      it('should contain "afterUpdate" callback', function() {
        assert(V1QuestionCode.callbacks['afterUpdate'], 'afterUpdate is missing.');
      });

      it('"afterUpdate" callback should be a function', function() {
      assert(_.isFunction(V1QuestionCode.callbacks['afterUpdate']), 'afterUpdate should be a function.');
    });
    
      

      it('should contain "beforeDestroy" callback', function() {
        assert(V1QuestionCode.callbacks['beforeDestroy'], 'beforeDestroy is missing.');
      });

      it('"beforeDestroy" callback should be a function', function() {
      assert(_.isFunction(V1QuestionCode.callbacks['beforeDestroy']), 'beforeDestroy should be a function.');
    });
    
      

      it('should contain "afterDestroy" callback', function() {
        assert(V1QuestionCode.callbacks['afterDestroy'], 'afterDestroy is missing.');
      });

      it('"afterDestroy" callback should be a function', function() {
      assert(_.isFunction(V1QuestionCode.callbacks['afterDestroy']), 'afterDestroy should be a function.');
    });
