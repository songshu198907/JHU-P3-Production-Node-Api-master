...
describe('V1JobAuditLogging Model', function() {

  afterEach(function() {
    V1JobAuditLogging.destroy();
  });
  ...
  it('should successfully create an instance of V1JobAuditLogging', function(done) {
    var beforeCount;

    V1JobAuditLogging.count()
      .then(function(count) {
        beforeCount = count;
        return V1JobAuditLogging.create({"createdAt":"1986-02-24 20:47:02 +0000","name":"Kathy Leannon","rowsEffected":"Weather subdivision"});
      })
      .then(function(obj) {
  ...
  it('should successfully read an instance of V1JobAuditLogging', function(done) {

    var attributes = {"createdAt":"2004-05-11 20:03:47 +0000","name":"Jacinto Vandervort","rowsEffected":"Teems noninterference"};
    var id;

    if (attributes && Object.keys(attributes).length !== 0) {
      V1JobAuditLogging.findOne(attributes)
        .then(function(record) {
          assert(!record, 'Should not have found a record.');
          return V1JobAuditLogging.create(attributes);
        })
        .then(function(obj) {
          assert(obj, 'Failed to create object.');
          id = obj.id;
          return V1JobAuditLogging.findOne({'id':obj.id});
        })
        .then(function(obj) {
  ...
  it('should successfully update an instance of V1JobAuditLogging', function(done) {
    var attributes = {"createdAt":"1986-03-13 22:28:23 +0000","name":"Echo Turner","rowsEffected":"Carbonation gentle"};
    var updatedObject;

    V1JobAuditLogging.create(attributes)
      .then(function(obj) {
        assert(obj, 'Failed to create object.');
        var attributes = {"createdAt":"2015-05-25 12:51:17 +0000","name":"Cindi Bergnaum","rowsEffected":"Multivariate caused"};
        return V1JobAuditLogging.update(obj.id, attributes);
      })
      .then(function(collection) {
        updatedObject = collection[0];
        assert(updatedObject, 'Failed to update object.');
        return V1JobAuditLogging.findOne({'id':updatedObject.id});
      })
      .then(function(obj) {
  ...
  it('should successfully destroy an instance of V1JobAuditLogging', function(done) {

    var attributes = {"createdAt":"2004-01-20 10:15:26 +0000","name":"Krysten Mraz","rowsEffected":"Superstitiously Riley"};
    ...
    if (attributes && Object.keys(attributes).length !== 0) {
      V1JobAuditLogging.findOne(attributes)
        ...
        .then(function(obj) {
          assert(obj, 'Failed to create object.');
          return V1JobAuditLogging.destroy(obj.id);
        })
        .then(function(collection) {
  ...
  describe('custom callbacks', function() {
    it('should have a callbacks object', function() {
      assert(V1JobAuditLogging.callbacks, 'V1JobAuditLogging is missing the callbacks object');
    });
      ...
      it('should contain "beforeCreate" callback', function() {
        assert(V1JobAuditLogging.callbacks['beforeCreate'], 'beforeCreate is missing.');
      });

      it('"beforeCreate" callback should be a function', function() {
      assert(_.isFunction(V1JobAuditLogging.callbacks['beforeCreate']), 'beforeCreate should be a function.');
    });
    
      

      it('should contain "afterCreate" callback', function() {
        assert(V1JobAuditLogging.callbacks['afterCreate'], 'afterCreate is missing.');
      });

      it('"afterCreate" callback should be a function', function() {
      assert(_.isFunction(V1JobAuditLogging.callbacks['afterCreate']), 'afterCreate should be a function.');
    });
    
      

      it('should contain "beforeFind" callback', function() {
        assert(V1JobAuditLogging.callbacks['beforeFind'], 'beforeFind is missing.');
      });

      it('"beforeFind" callback should be a function', function() {
      assert(_.isFunction(V1JobAuditLogging.callbacks['beforeFind']), 'beforeFind should be a function.');
    });
    
      

      it('should contain "afterFind" callback', function() {
        assert(V1JobAuditLogging.callbacks['afterFind'], 'afterFind is missing.');
      });

      it('"afterFind" callback should be a function', function() {
      assert(_.isFunction(V1JobAuditLogging.callbacks['afterFind']), 'afterFind should be a function.');
    });
    
      

      it('should contain "beforeUpdate" callback', function() {
        assert(V1JobAuditLogging.callbacks['beforeUpdate'], 'beforeUpdate is missing.');
      });

      it('"beforeUpdate" callback should be a function', function() {
      assert(_.isFunction(V1JobAuditLogging.callbacks['beforeUpdate']), 'beforeUpdate should be a function.');
    });
    
      

      it('should contain "afterUpdate" callback', function() {
        assert(V1JobAuditLogging.callbacks['afterUpdate'], 'afterUpdate is missing.');
      });

      it('"afterUpdate" callback should be a function', function() {
      assert(_.isFunction(V1JobAuditLogging.callbacks['afterUpdate']), 'afterUpdate should be a function.');
    });
    
      

      it('should contain "beforeDestroy" callback', function() {
        assert(V1JobAuditLogging.callbacks['beforeDestroy'], 'beforeDestroy is missing.');
      });

      it('"beforeDestroy" callback should be a function', function() {
      assert(_.isFunction(V1JobAuditLogging.callbacks['beforeDestroy']), 'beforeDestroy should be a function.');
    });
    
      

      it('should contain "afterDestroy" callback', function() {
        assert(V1JobAuditLogging.callbacks['afterDestroy'], 'afterDestroy is missing.');
      });

      it('"afterDestroy" callback should be a function', function() {
      assert(_.isFunction(V1JobAuditLogging.callbacks['afterDestroy']), 'afterDestroy should be a function.');
    });
