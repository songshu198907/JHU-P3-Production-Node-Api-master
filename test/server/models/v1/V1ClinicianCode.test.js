...
describe('V1ClinicianCode Model', function() {

  afterEach(function() {
    V1ClinicianCode.destroy();
  });
  ...
  it('should successfully create an instance of V1ClinicianCode', function(done) {
    var beforeCount;

    V1ClinicianCode.count()
      .then(function(count) {
        beforeCount = count;
        return V1ClinicianCode.create({"code":"Commits toning","name":"Janyce Brekke"});
      })
      .then(function(obj) {
        assert(obj, 'Failed to create object.');
        return V1ClinicianCode.count();
      })
      .then(function(count) {
  ...
  it('should successfully read an instance of V1ClinicianCode', function(done) {

    var attributes = {"code":"Fingerboards footballers","name":"Katherin Quitzon"};
    var id;

    if (attributes && Object.keys(attributes).length !== 0) {
      V1ClinicianCode.findOne(attributes)
        .then(function(record) {
          assert(!record, 'Should not have found a record.');
          return V1ClinicianCode.create(attributes);
        })
        .then(function(obj) {
          assert(obj, 'Failed to create object.');
          id = obj.id;
          return V1ClinicianCode.findOne({'id':obj.id});
        })
        .then(function(obj) {
  ...
  it('should successfully update an instance of V1ClinicianCode', function(done) {
    var attributes = {"code":"Differentiating hunchbacked","name":"Cletus Wilkinson"};
    var updatedObject;

    V1ClinicianCode.create(attributes)
      .then(function(obj) {
        assert(obj, 'Failed to create object.');
        var attributes = {"code":"Sucrose superconductors","name":"Summer Jewess"};
        return V1ClinicianCode.update(obj.id, attributes);
      })
      .then(function(collection) {
        updatedObject = collection[0];
        assert(updatedObject, 'Failed to update object.');
        return V1ClinicianCode.findOne({'id':updatedObject.id});
      })
      .then(function(obj) {
  ...
  it('should successfully destroy an instance of V1ClinicianCode', function(done) {

    var attributes = {"code":"Crosby cliquish","name":"Darin Brekke"};
    ...
    if (attributes && Object.keys(attributes).length !== 0) {
      V1ClinicianCode.findOne(attributes)
        ...
        .then(function(obj) {
          assert(obj, 'Failed to create object.');
          return V1ClinicianCode.destroy(obj.id);
        })
        .then(function(collection) {
  ...
  describe('custom callbacks', function() {
    it('should have a callbacks object', function() {
      assert(V1ClinicianCode.callbacks, 'V1ClinicianCode is missing the callbacks object');
    });
      ...
      it('should contain "beforeCreate" callback', function() {
        assert(V1ClinicianCode.callbacks['beforeCreate'], 'beforeCreate is missing.');
      });

      it('"beforeCreate" callback should be a function', function() {
      assert(_.isFunction(V1ClinicianCode.callbacks['beforeCreate']), 'beforeCreate should be a function.');
    });
    
      

      it('should contain "afterCreate" callback', function() {
        assert(V1ClinicianCode.callbacks['afterCreate'], 'afterCreate is missing.');
      });

      it('"afterCreate" callback should be a function', function() {
      assert(_.isFunction(V1ClinicianCode.callbacks['afterCreate']), 'afterCreate should be a function.');
    });
    
      

      it('should contain "beforeFind" callback', function() {
        assert(V1ClinicianCode.callbacks['beforeFind'], 'beforeFind is missing.');
      });

      it('"beforeFind" callback should be a function', function() {
      assert(_.isFunction(V1ClinicianCode.callbacks['beforeFind']), 'beforeFind should be a function.');
    });
    
      

      it('should contain "afterFind" callback', function() {
        assert(V1ClinicianCode.callbacks['afterFind'], 'afterFind is missing.');
      });

      it('"afterFind" callback should be a function', function() {
      assert(_.isFunction(V1ClinicianCode.callbacks['afterFind']), 'afterFind should be a function.');
    });
    
      

      it('should contain "beforeUpdate" callback', function() {
        assert(V1ClinicianCode.callbacks['beforeUpdate'], 'beforeUpdate is missing.');
      });

      it('"beforeUpdate" callback should be a function', function() {
      assert(_.isFunction(V1ClinicianCode.callbacks['beforeUpdate']), 'beforeUpdate should be a function.');
    });
    
      

      it('should contain "afterUpdate" callback', function() {
        assert(V1ClinicianCode.callbacks['afterUpdate'], 'afterUpdate is missing.');
      });

      it('"afterUpdate" callback should be a function', function() {
      assert(_.isFunction(V1ClinicianCode.callbacks['afterUpdate']), 'afterUpdate should be a function.');
    });
    
      

      it('should contain "beforeDestroy" callback', function() {
        assert(V1ClinicianCode.callbacks['beforeDestroy'], 'beforeDestroy is missing.');
      });

      it('"beforeDestroy" callback should be a function', function() {
      assert(_.isFunction(V1ClinicianCode.callbacks['beforeDestroy']), 'beforeDestroy should be a function.');
    });
    
      

      it('should contain "afterDestroy" callback', function() {
        assert(V1ClinicianCode.callbacks['afterDestroy'], 'afterDestroy is missing.');
      });

      it('"afterDestroy" callback should be a function', function() {
      assert(_.isFunction(V1ClinicianCode.callbacks['afterDestroy']), 'afterDestroy should be a function.');
    });
