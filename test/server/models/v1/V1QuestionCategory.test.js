...
describe('V1QuestionCategory Model', function() {

  afterEach(function() {
    V1QuestionCategory.destroy();
  });
  ...
  it('should successfully create an instance of V1QuestionCategory', function(done) {
    var beforeCount;

    V1QuestionCategory.count()
      .then(function(count) {
        beforeCount = count;
        return V1QuestionCategory.create({"name":"Annabelle Kertzmann"});
      })
      .then(function(obj) {
  ...
  it('should successfully read an instance of V1QuestionCategory', function(done) {

    var attributes = {"name":"Millicent Wunsch"};
    var id;

    if (attributes && Object.keys(attributes).length !== 0) {
      V1QuestionCategory.findOne(attributes)
        .then(function(record) {
          assert(!record, 'Should not have found a record.');
          return V1QuestionCategory.create(attributes);
        })
        .then(function(obj) {
          assert(obj, 'Failed to create object.');
          id = obj.id;
          return V1QuestionCategory.findOne({'id':obj.id});
        })
        .then(function(obj) {
  ...
  it('should successfully update an instance of V1QuestionCategory', function(done) {
    var attributes = {"name":"Alfred Stokes"};
    var updatedObject;

    V1QuestionCategory.create(attributes)
      .then(function(obj) {
        assert(obj, 'Failed to create object.');
        var attributes = {"name":"Edda Stanton"};
        return V1QuestionCategory.update(obj.id, attributes);
      })
      .then(function(collection) {
        updatedObject = collection[0];
        assert(updatedObject, 'Failed to update object.');
        return V1QuestionCategory.findOne({'id':updatedObject.id});
      })
      .then(function(obj) {
  ...
  it('should successfully destroy an instance of V1QuestionCategory', function(done) {

    var attributes = {"name":"Desmond Kub"};
    ...
    if (attributes && Object.keys(attributes).length !== 0) {
      V1QuestionCategory.findOne(attributes)
        ...
        .then(function(obj) {
          assert(obj, 'Failed to create object.');
          return V1QuestionCategory.destroy(obj.id);
        })
        .then(function(collection) {
  ...
  describe('custom callbacks', function() {
    it('should have a callbacks object', function() {
      assert(V1QuestionCategory.callbacks, 'V1QuestionCategory is missing the callbacks object');
    });
      ...
      it('should contain "beforeCreate" callback', function() {
        assert(V1QuestionCategory.callbacks['beforeCreate'], 'beforeCreate is missing.');
      });

      it('"beforeCreate" callback should be a function', function() {
      assert(_.isFunction(V1QuestionCategory.callbacks['beforeCreate']), 'beforeCreate should be a function.');
    });
    
      

      it('should contain "afterCreate" callback', function() {
        assert(V1QuestionCategory.callbacks['afterCreate'], 'afterCreate is missing.');
      });

      it('"afterCreate" callback should be a function', function() {
      assert(_.isFunction(V1QuestionCategory.callbacks['afterCreate']), 'afterCreate should be a function.');
    });
    
      

      it('should contain "beforeFind" callback', function() {
        assert(V1QuestionCategory.callbacks['beforeFind'], 'beforeFind is missing.');
      });

      it('"beforeFind" callback should be a function', function() {
      assert(_.isFunction(V1QuestionCategory.callbacks['beforeFind']), 'beforeFind should be a function.');
    });
    
      

      it('should contain "afterFind" callback', function() {
        assert(V1QuestionCategory.callbacks['afterFind'], 'afterFind is missing.');
      });

      it('"afterFind" callback should be a function', function() {
      assert(_.isFunction(V1QuestionCategory.callbacks['afterFind']), 'afterFind should be a function.');
    });
    
      

      it('should contain "beforeUpdate" callback', function() {
        assert(V1QuestionCategory.callbacks['beforeUpdate'], 'beforeUpdate is missing.');
      });

      it('"beforeUpdate" callback should be a function', function() {
      assert(_.isFunction(V1QuestionCategory.callbacks['beforeUpdate']), 'beforeUpdate should be a function.');
    });
    
      

      it('should contain "afterUpdate" callback', function() {
        assert(V1QuestionCategory.callbacks['afterUpdate'], 'afterUpdate is missing.');
      });

      it('"afterUpdate" callback should be a function', function() {
      assert(_.isFunction(V1QuestionCategory.callbacks['afterUpdate']), 'afterUpdate should be a function.');
    });
    
      

      it('should contain "beforeDestroy" callback', function() {
        assert(V1QuestionCategory.callbacks['beforeDestroy'], 'beforeDestroy is missing.');
      });

      it('"beforeDestroy" callback should be a function', function() {
      assert(_.isFunction(V1QuestionCategory.callbacks['beforeDestroy']), 'beforeDestroy should be a function.');
    });
    
      

      it('should contain "afterDestroy" callback', function() {
        assert(V1QuestionCategory.callbacks['afterDestroy'], 'afterDestroy is missing.');
      });

      it('"afterDestroy" callback should be a function', function() {
      assert(_.isFunction(V1QuestionCategory.callbacks['afterDestroy']), 'afterDestroy should be a function.');
    });
