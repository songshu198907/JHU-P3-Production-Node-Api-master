...
var fakeUsers = {
  'administrator': {"actualChildBirth":"2014-09-23","address":"Congratulations spiels","cellPhone":"Disinclined dreadnought","city":"Audited Singleton","clinicianCodeId":78130,"clinicName":"Metamorphosed pallbearers","consentAcceptedOn":"1971-11-11 18:53:00 +0000","contactCellPhone":"Kookaburras rectification","contactEmail":"Outmanoeuvring framework","contactHomePhone":"Antiparticles mosey","contactName":"Adduce synthesizers","deactivatedOn":"2007-06-07 00:55:34 +0000","educationId":16914,"email":"ignacio.turcotte88@vaccinesurvey.com","expectedChildBirth":"2015-07-03","firstName":"Aliza","hasContactUsers":true,"homePhone":"Jitterbugging impositions","interventionGroup":"Archipelagos disclosures","isDeactive":true,"lastName":"Parker","parentRelationshipType":"Beltway midge","parentUserId":82532,"password":"password","passwordConfirmation":"password","passwordDigest":"Henpecks zincs","patientType":32542,"postalCode":"Scholastically blotch","raceId":4565,"reasonForDeactivation":"Youngster Horthy","resetPassword":false,"role":"administrator","state":"Hookah addicting","updatedUserProfile":false,"vaccinationReminders":false,"xSessionId":"Stereotyped improvisations"},
};
...
describe('V2QuestionCategory', function() {
  
    beforeEach(function (done){
      agent = superagent.agent();
      objects = [];
var testObjects = [];
  testObjects.push({"name":"Lane Dickens"});
  testObjects.push({"name":"Karl Zboncak"});
  testObjects.push({"name":"Viva Reynolds"});
  testObjects.push({"name":"Clemente Thompson"});
  testObjects.push({"name":"Morris Anderson"});
  testObjects.push({"name":"Tennie Mayert"});
  testObjects.push({"name":"Elmo Schaefer"});
  testObjects.push({"name":"Alyssa Kohler"});
  testObjects.push({"name":"Jeane Kunze"});
  testObjects.push({"name":"Daron Walker"});
...
V2QuestionCategory.createEach(testObjects).exec(function(err, obj) {
  ...
  before(function() {
    ...
    it('should attempt to get index of question_categories for administrator', function (done) {
      ...
      agent.get("http://localhost:" + (process.env.TEST_PORT || 1337) + "/api/v2/question_categories").set('Content-Type', 'application/json').end(function (err, res) {
    ...
    it('should attempt to create question_category for administrator', function (done) {
      var newObj = {"name":"Cyrus Daniel"};
  
      passportStub.login(fakeUsers['administrator']);
  
      agent.post("http://localhost:" + (process.env.TEST_PORT || 1337) + "/api/v2/question_categories").send(newObj).set('Content-Type', 'application/json').end(function (err, res) {
  
        if (err) return done(err);
        assert.equal(res.status, 201);
        
          V2QuestionCategory.count().exec(function (err, count) {
            assert(!err, "Received error " + util.inspect(err, {depth: null}));
    ...
    it('should attempt to show question_category for administrator', function(done) {
      ...
      agent.get("http://localhost:" + (process.env.TEST_PORT || 1337) + "/api/v2/question_categories/" + id).set('Content-Type', 'application/json').end(function (err, res) {
    ...
    it('should attempt to update question_category for administrator', function (done) {
      var id = objects[0].id;
      var newObj = {"name":"Tory Conroy"};
      newObj.id = id;
  
      passportStub.login(fakeUsers['administrator']);
  
      agent.put("http://localhost:" + (process.env.TEST_PORT || 1337) + "/api/v2/question_categories/" + id).send(newObj).set('Content-Type', 'application/json').end(function (err, res) {
  
        if (err) return done(err);
        assert.equal(res.status, 204);
        
          V2QuestionCategory.findOne({id: id}).exec(function(err, obj) {
            if (err) return done(err);
            assert(obj, 'Expected to get V2QuestionCategory with id ' + id);
            done();
          });
    ...
    it('should attempt to destroy question_category for administrator', function (done) {
      ...
      agent.del("http://localhost:" + (process.env.TEST_PORT || 1337) + "/api/v2/question_categories/" + id).end(function (err, res) {
        if (err) return done(err);
